...
testcase external_sandra_weekly_visit_native_beats:
    ...
    advance until screen "choice" timeout 20.0
    assert eval (str(main_ui_runtime.action_title or "") == "Разговор на кухне" and "распускают руки" in str(scene_runtime.text or "")) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("часть работы" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("Стефан здесь хозяин" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("подумаете, как защитить" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (threads["sandraKitchenHouseholdRespect"].completed and renpy.get_screen("choice") is None) timeout 10.0
...
testcase external_sandra_breakfast_flirt_date_destination:
    ...
    $ Amanda.set_sex_stat("sexacts", 0)
    assert eval (not Amanda.liza_sex_guidance_received() and str(tavern_breakfast_tease_candidate().get("girl", "") or "") == "") timeout 5.0
    ...
    assert eval (all(option in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] for option in ["Попросить помочь рукой", "Попросить сделать минет", "Кончить..."])) timeout 5.0
    ...
    click id ("choice_panel_button_%d" % int(_sandra_oral_index)) pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and Sandra.cock_in("mouth") and "Кончить..." in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    ...
    click id _sandra_enter_button pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and Sandra.cock_in("pussy") and "Кончить..." in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    $ _sandra_finish_index = [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])].index("Кончить...")
    click id ("choice_panel_button_%d" % int(_sandra_finish_index)) pos (0.5, 0.5)
    advance until eval (renpy.get_screen("choice") is not None and "Кончить..." not in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] and "Назад" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    assert eval ("Кончить на грудь" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    assert eval ("Кончить на лицо" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    assert eval ("Кончить в киску" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    ...
    advance until eval (renpy.get_screen("choice") is not None and "Кончить..." in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    $ _sandra_anal_menu_index = [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])].index("Кончить...")
    click id ("choice_panel_button_%d" % int(_sandra_anal_menu_index)) pos (0.5, 0.5)
    advance until eval (renpy.get_screen("choice") is not None and "Кончить..." not in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] and "Назад" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    assert eval ("Кончить на грудь" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    assert eval ("Кончить на лицо" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    assert eval ("Кончить в попку" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    ...
    $ Amanda.rel = 11
    $ Amanda.corruption = 45
    run Call("TavernKitchenBreakfastTeasePrivate", "amanda", "player_room")
    advance until screen "choice" timeout 20.0
    assert eval ([str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] == ["Продолжить свидание"] and str(rooms.current_code or "") == "TavernMyRoom") timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and "Закончить" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    $ _amanda_date_finish_index = [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])].index("Закончить")
    click id ("choice_panel_button_%d" % int(_amanda_date_finish_index)) pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] == ["Закончить свидание"]) timeout 20.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (renpy.get_screen("choice") is None and str(rooms.current_code or "") == "TavernMyRoom") timeout 20.0
    ...
    advance until screen "choice" timeout 20.0
    assert eval ([str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] == ["Продолжить прогулку"] and str(rooms.current_code or "") == "ForestLake") timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] == ["Закончить свидание"]) timeout 20.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (renpy.get_screen("choice") is None and str(rooms.current_code or "") == "ForestLake") timeout 20.0
...
testcase external_georgette_back_alley_not_visible_in_port_streets:
    ...
    $ Georgett.rel = 7
    $ Georgett.talked_today = 0
    $ Georgett.set_story_value("seeclients", 0)
    $ Georgett.set_story_value("TalkChurchAfterCermonLiza", 0)
    $ Georgett.detailed_sex_history = []
    $ TodaySexEvents_Clear()
    $ TodaySexEvents_Add("georgett", 3, 1, "Prostitution")
    $ initStoryEventRuntime(True)
    assert eval (not Georgett.portstreet_client_event_available()) timeout 5.0
    $ Georgett.record_sex_history("You", "street", "inside")
    $ initStoryEventRuntime(True)
    assert eval (Georgett.portstreet_client_event_available()) timeout 5.0
    run Jump("PortStreets")
    advance until screen "main_ui" timeout 20.0
    ...
    click id ("choice_panel_button_%d" % int(_georgett_alley_index)) pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and "Вернуться в переулок" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    assert eval (renpy.get_screen("main_ui") is not None and str(main_ui_runtime.mode or "") == "event") timeout 5.0
    assert eval ("Подсмотреть" not in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] and "Смотреть дальше" not in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    assert eval ("Вы решили пойти и проверить подворотню" in str(scene_runtime.text or "") and "Вы видите стоящую раком Жоржетту" in str(scene_runtime.text or "") and str(scene_runtime.picture or "").startswith("images/georgett/portevents/event1_")) timeout 5.0
    ...
    assert eval ("Спросить о клиентах" in [str(getattr(i, "caption", "") or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    assert eval (str(people.location("georgett") or "") == "PortStreets") timeout 5.0
...
testcase external_melissa_bats_room_search_after_wait:
    ...
    assert eval (int(threads["melissaBatProblem"].data.length or 0) == 12) timeout 5.0
    assert eval (int(threads["melissaBatProblem"].num or 0) == 6) timeout 5.0
    assert eval ([str(evt.target or "") for evt in threads["melissaBatProblem"].data.triggers[6]] == ["story_melissa_bat_problem_4"]) timeout 5.0
    assert eval (not story_event_available("TavernMelissaRoom", "room_search")) timeout 5.0
    ...
    $ external_calendar_set_fields(int(_melissa_breakfast_date.get("day", 1) or 1), int(_melissa_breakfast_date.get("month", 1) or 1), int(_melissa_breakfast_date.get("year", CALENDAR_START_CYCLE) or CALENDAR_START_CYCLE), 8, 0)
    $ player.tavern_management.breakfast.today = False
    $ player.tavern_management.breakfast.event_active = False
    $ rooms.enter("TavernKitchen")
    $ event_runtime.evaluation_time = None
    $ findAvailableEvents(True)
    assert eval (story_event_available("TavernKitchen", "enter")) timeout 5.0
    assert eval (str(event_runtime.available["TavernKitchen"]["enter"].target or "") == "story_melissa_bat_problem_breakfast_argument") timeout 5.0
    run Jump("TavernKitchen")
    advance until screen "choice" timeout 20.0
    assert eval (str(scene_runtime.picture or "") == "images/breakfast/tavent_girls.jpg") timeout 5.0
    assert eval ("как и обещала" in str(scene_runtime.text or "")) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (int(threads["melissaBatProblem"].num or 0) == 9) timeout 25.0
    assert eval (bool(player.tavern_management.breakfast.today)) timeout 5.0

    $ rooms.enter("TavernAmandaRoom")
    $ event_runtime.evaluation_time = None
    $ findAvailableEvents(True)
    assert eval (story_event_available("TavernAmandaRoom", "melissa_bats")) timeout 5.0
    run Jump("TavernAmandaRoom")
    advance until screen "choice" timeout 20.0
    assert eval (str(scene_runtime.picture or "").endswith("underBedsearch_0.png")) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (str(scene_runtime.picture or "").endswith("underBedsearch_1.png")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (str(scene_runtime.picture or "").endswith("underBedsearch_2.png")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (int(threads["melissaBatProblem"].num or 0) == 10) timeout 10.0
    assert eval (str(Melissa.temp_room_code or "") == "") timeout 5.0

    $ rooms.enter("TavernMelissaRoom")
    $ event_runtime.evaluation_time = None
    $ findAvailableEvents(True)
    assert eval (story_event_available("TavernMelissaRoom", "room_search")) timeout 5.0
    assert eval (str(event_runtime.available["TavernMelissaRoom"]["room_search"].target or "") == "story_melissa_bat_problem_booklet_search") timeout 5.0
    run Jump("TavernMelissaRoom")
    advance until screen "main_ui" timeout 20.0
    ...
    assert eval (int(threads["melissaBatProblem"].num or 0) == 11) timeout 5.0
    ...
    $ Melissa.rel = max(10, int(Melissa.rel or 0))
    $ Melissa.openness = max(4, int(Melissa.openness or 0))
    $ Melissa.corruption = max(8, int(Melissa.corruption or 0))
    $ rooms.enter("TavernMain")
    $ event_runtime.evaluation_time = None
    $ findAvailableEvents(True)
    ...
    advance until screen "choice" timeout 20.0
    assert eval ("Спасибо" in str(scene_runtime.text or "")) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (int(threads["melissaBatProblem"].num or 0) == 12) timeout 15.0
...
testcase external_melissa_bats_existing_booklet_after_roof:
    ...
    $ external_calendar_set_fields(int(_melissa_bats_test_date.get("day", 1) or 1), int(_melissa_bats_test_date.get("month", 1) or 1), int(_melissa_bats_test_date.get("year", CALENDAR_START_CYCLE) or CALENDAR_START_CYCLE), 8, 0)
    $ BlockTimeAdvance = 0
    $ TavernEventOngoing = ""
    $ main_ui_runtime.overlay = ""
    $ main_ui_runtime.action_content = None
    ...
    $ event_runtime.evaluation_time = None
    $ initStoryEventRuntime(True)
    $ threads["melissaBatProblem"].advanceTo(9, force_active=True)
    $ findAvailableEvents(True)
    assert eval (story_event_available("TavernAmandaRoom", "melissa_bats")) timeout 5.0
    run Call("checkTriggers", "TavernAmandaRoom", "melissa_bats", 0)
    advance until screen "choice" timeout 20.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (str(scene_runtime.picture or "").endswith("underBedsearch_1.png")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (str(scene_runtime.picture or "").endswith("underBedsearch_2.png")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (int(threads["melissaBatProblem"].num or 0) == 11) timeout 10.0
    assert eval (bool(Melissa.drawings_found) and str(Melissa.temp_room_code or "") == "") timeout 5.0

testcase external_melissa_v72_breakfast_stage_migration:
    run Call("InitGameNPCs")
    $ threads["melissaBatProblem"].advanceTo(8, force_active=True)
    $ Melissa.drawings_found = False
    $ updateSave_V72()
    assert eval (int(threads["melissaBatProblem"].num or 0) == 10 and not threads["melissaBatProblem"].completed) timeout 5.0

    $ threads["melissaBatProblem"].advanceTo(9, force_active=True)
    $ updateSave_V72()
    assert eval (int(threads["melissaBatProblem"].num or 0) == 11 and not threads["melissaBatProblem"].completed) timeout 5.0

    $ threads["melissaBatProblem"].advanceTo(10, force_active=True)
    $ threads["melissaBatProblem"].completed = True
    $ updateSave_V72()
    assert eval (int(threads["melissaBatProblem"].num or 0) == int(threads["melissaBatProblem"].data.length or 0) and threads["melissaBatProblem"].completed) timeout 5.0
...
testcase external_amanda_talk_opens_from_npc_button:
    ...
    click id _amanda_flirt_button_id pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and str(main_ui_runtime.mode or "") == "talk" and int(Amanda.flirted_today or 0) == _amanda_flirted + 1) timeout 20.0
    ...
    assert eval (str(rooms.current_code or "") == "TavernMain" and "Назад" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
...
testcase external_sandra_pending_soap_request_can_be_fulfilled:
    ...
    $ people.get_data("sandra").set_schedule([NPCScheduleEntry(location="TavernKitchen", start_minute=0, end_minute=1440, priority=999)])
    $ rooms.enter("TavernKitchen")
    $ crafting.soap_requests["sandra"] = 1
    $ player.add_item("lavender_herbal_soap_001", 2)
    $ _sandra_soap_before = int(player.item_count("lavender_herbal_soap_001") or 0)
    run Call("IntSandraTalk", "sandra")
    advance until eval (renpy.get_screen("choice") is not None and "Отдать Сандре обещанное мыло" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    $ _sandra_soap_action_index = [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])].index("Отдать Сандре обещанное мыло")
    click id ("choice_panel_button_%d" % int(_sandra_soap_action_index)) pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and "Отдать лавандово-травяное мыло" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] and "Назад" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    $ _sandra_soap_back_index = [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])].index("Назад")
    click id ("choice_panel_button_%d" % int(_sandra_soap_back_index)) pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and "Отдать Сандре обещанное мыло" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    assert eval (int(player.item_count("lavender_herbal_soap_001") or 0) == _sandra_soap_before and int(crafting.soap_requests.get("sandra", 0) or 0) == 1) timeout 5.0
    $ _sandra_soap_action_index = [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])].index("Отдать Сандре обещанное мыло")
    click id ("choice_panel_button_%d" % int(_sandra_soap_action_index)) pos (0.5, 0.5) until eval (renpy.get_screen("choice") is not None and "Отдать лавандово-травяное мыло" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    $ _sandra_soap_give_index = [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])].index("Отдать лавандово-травяное мыло")
    click id ("choice_panel_button_%d" % int(_sandra_soap_give_index)) pos (0.5, 0.5)
    advance until eval (renpy.get_screen("choice") is not None and "Назад" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] and "Отдать Сандре обещанное мыло" not in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 20.0
    assert eval (int(player.item_count("lavender_herbal_soap_001") or 0) == _sandra_soap_before - 1) timeout 5.0
    assert eval ("sandra" not in crafting.soap_requests) timeout 5.0
    assert eval ("принимает подарок" in str(scene_runtime.text or "")) timeout 5.0
...
testcase external_sandra_weekly_thread_progression:
    ...
    python:
        ...
        for _melissa_field in (
            ...
            "storage_rat_help_day", "bat_attic_check_day",
    ...
    assert eval (Melissa.storage_rat_help_day == 15 and Melissa.bat_attic_check_day == 16 and not hasattr(Melissa, "drawings_ready_day") and Melissa.drawings_found and Melissa.drawings_booklet_left and Melissa.drawings_booklet_read and Melissa.drawings_returned) timeout 5.0
...
testcase external_sandra_night_thanks_hours_work:
    ...
    $ main_ui_runtime.action_items = tavern_sandra_room_action_items()
    assert eval ([str(i.caption or "") for i in main_ui_runtime.action_items if "Санд" in str(i.caption or "")] == ["Заняться сексом с Сандрой"]) timeout 5.0
...
testcase external_melissa_courtship_is_slow_and_daily:
    ...
    $ external_calendar_set_fields(int(_melissa_courtship_date.get("day", 1) or 1), int(_melissa_courtship_date.get("month", 1) or 1), int(_melissa_courtship_date.get("year", CALENDAR_START_CYCLE) or CALENDAR_START_CYCLE), 14, 0)
    $ external_calendar_set_weekday(2)
    $ rooms.enter("TavernMain")
    $ people.get_data("amanda").set_schedule([NPCScheduleEntry(location="TavernMain", start_hour=0, end_hour=24, priority=999, working=True)])
    $ people.get_data("melissa").set_schedule([NPCScheduleEntry(location="TavernMain", start_hour=0, end_hour=24, priority=999, working=True)])
    $ people.get_data("liza").set_schedule([NPCScheduleEntry(location="TavernMain", start_hour=0, end_hour=24, priority=999, working=True)])
    $ Liza.set_hired(True)
    $ Amanda.set_var_int("lizafriends", 1)
    $ Amanda.set_sex_stat("sexacts", 1)
    $ Melissa.rel = 16
    $ Melissa.openness = 12
    ...
    advance until screen "choice" timeout 20.0
    assert eval ("Аманда вполголоса рассказывает Мелиссе" in str(scene_runtime.text or "") and str(scene_runtime.picture or "") == MelissaStaticData.image_path("courtship", "amanda_talk")) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("Я раньше думала" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("Мелисса сначала отшучивается" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (int(threads["melissaCourtship"].num or 0) == 1 and renpy.get_screen("choice") is None) timeout 20.0
    assert eval (int(Melissa.fucked_today or 0) == 0 and not any(str(evt.target or "") == "story_melissa_courtship_storm_1" for evt in threads["melissaCourtship"].getAvailableEvents())) timeout 5.0

    $ _melissa_next_date = calendar_v2.day_number_to_parts(current_game_day() + 1)
    $ external_calendar_set_fields(int(_melissa_next_date.get("day", 1) or 1), int(_melissa_next_date.get("month", 1) or 1), int(_melissa_next_date.get("year", CALENDAR_START_CYCLE) or CALENDAR_START_CYCLE), 22, 0)
    $ rooms.enter("TavernMyRoom")
    $ initStoryEventRuntime(True)
    assert eval (story_event_available("TavernMyRoom", "sleep") and str(event_runtime.available["TavernMyRoom"]["sleep"].target or "") == "story_melissa_courtship_storm_1") timeout 5.0

    run Call("checkTriggers", "TavernMyRoom", "sleep", 0)
    advance until screen "choice" timeout 20.0
    assert eval ("грохочет гроза" in str(scene_runtime.text or "") and "Разрешить Мелиссе остаться" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("уступаете Мелиссе край кровати" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("ласкает себя" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (int(threads["melissaCourtship"].num or 0) == 2 and renpy.get_screen("choice") is None) timeout 20.0
    ...
    run Call("checkTriggers", "TavernMyRoom", "sleep", 0)
    advance until screen "choice" timeout 20.0
    assert eval ("гроза уже ни при чем" in str(scene_runtime.text or "")) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("каждый ласкает себя сам" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (int(threads["melissaCourtship"].num or 0) == 3 and renpy.get_screen("choice") is None) timeout 20.0
    ...
    run Call("checkTriggers", "TavernMyRoom", "sleep", 0)
    advance until screen "choice" timeout 20.0
    assert eval ("можно ли ей самой коснуться" in str(scene_runtime.text or "")) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("обхватывает ствол ладонью" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (int(threads["melissaCourtship"].num or 0) == 4 and renpy.get_screen("choice") is None) timeout 20.0
    ...
    run Call("checkTriggers", "TavernMyRoom", "sleep", 0)
    advance until screen "choice" timeout 20.0
    assert eval ("попробовать его на вкус" in str(scene_runtime.text or "")) timeout 5.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval ("касается губами головки" in str(scene_runtime.text or "")) timeout 10.0
    click id "choice_panel_button_0" pos (0.5, 0.5) until eval (threads["melissaCourtship"].completed and renpy.get_screen("choice") is None) timeout 20.0
    assert eval (int(Melissa.fucked_today or 0) == 0 and Melissa.relationship_allows("intimacy")) timeout 5.0
...
testcase external_melissa_finished_intimacy_returns_to_room_and_allows_second_visit:
    ...
    $ updateSave_V74()
    assert eval (not hasattr(Melissa, "gift_preferences") and not hasattr(Sandra, "gift_preferences") and "sandraKitchenHouseholdRespect" in threads) timeout 5.0
'''
...
testcase external_harassment_event_picture_sequence:
    ...
    $ Melissa.set_harass_instruction("")
    $ _harass_eject_fame = int(player.economy.tavern_fame or 0)
    run Call("IntHarrassmentDiscuss", "melissa", 1)
    advance until screen "choice" timeout 20.0
    assert eval ("Сказать, что вы обдумаете проблему" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])] and "Выгнать наглого клиента" in [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])]) timeout 5.0
    $ _harass_eject_index = [str(i.caption or "") for i in renpy.get_screen("choice").scope.get("items", [])].index("Выгнать наглого клиента")
