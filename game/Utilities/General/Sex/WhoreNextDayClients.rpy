...
label WhoreNextDayClients(girl_name="", max_clients=0, glory_hole_max=0):
    ...
    $ _wnd_girl = getPersonInfo(_wnd_girl_name)
    ...
    python:
        ...
        if people_to_int(_wnd_girl.job_value("jobgloryholeTommorow", 0), 0) != 0:
            generated_clients = (glory_max_i * (75 + 5 * procedural_randint(1, 10, "glory_clients_%s_%s" % (_wnd_girl_name, int(dayspassed or 0))))) // 100
            _wnd_girl.set_sex_stat("clients_day_total", generated_clients)
            created = 0
            safety = 0
            while created < generated_clients and safety < 400:
                safety += 1
                event_hour = procedural_randint(2, 3, "glory_time_%s_%s_%s" % (_wnd_girl_name, int(dayspassed or 0), safety))
                if week_val == 5:
                    event_hour = 2
                if week_val == 7:
                    event_hour = 3

                event_type = procedural_randint(1, 11, "glory_type_%s_%s_%s" % (_wnd_girl_name, int(dayspassed or 0), safety))
            ...
            if max_clients_i > 0:
                generated_clients = procedural_randint(1, max_clients_i, "port_clients_%s_%s" % (_wnd_girl_name, int(dayspassed or 0)))
            else:
                generated_clients = 0
            if week_val == 5:
                generated_clients = 0
            ...
            while created < generated_clients:
                event_type = procedural_randint(1, prostitution_max_type, "port_client_type_%s_%s_%s" % (_wnd_girl_name, int(dayspassed or 0), created))
