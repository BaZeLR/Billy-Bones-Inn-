...
label NextDay(retlocname, timepassed):
    ...
    if player.economy.money == 0 or player.tavern_management.visitors == 0:
        ...
        python:
            for _day_start_stale_slot in list(renpy.list_saved_games(r"^quick-[0-9]+$", fast=True) or []):
                _day_start_slot_number = int(str(_day_start_stale_slot).split("-", 1)[1])
                if _day_start_slot_number > int(config.quicksave_slots):
                    renpy.unlink_save(_day_start_stale_slot)
