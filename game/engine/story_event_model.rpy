...
init python:
    ...
    def _story_num_day():
        return ((_story_to_int(year, 1100) - 1100) * 372) + ((_story_to_int(month, 1) - 1) * 31) + _story_to_int(day, 1)

    def _story_level_enabled(level):
        levels_map = globals().get("story_thread_levels", {})
    ...
    def _story_eval_condition(expr):
        ...
        if text.startswith("#"):
            text = text[1:].strip()
        if text in globals():
            return bool(globals()[text])
        try:
            return bool(eval(text, {"__builtins__": __builtins__}, globals()))
        except Exception:
            return False
    ...
    def _story_reference_day(ref_name, fallback=0):
        ...
        value = globals().get(ref, fallback)
        return _story_to_int(value, fallback)
    ...
    class Event(object):
        ...
        def checkNumDay(self, evt_numDay):
            if self.evtDay is None:
                return True
            if isinstance(self.evtDay, int):
                delta = int(self.evtDay)
                return _story_num_day() >= _story_to_int(evt_numDay, 0) + delta

            if isinstance(self.evtDay, tuple):
                evt_day_name, delta = self.evtDay
            else:
                evt_day_name, delta = (self.evtDay, 1)

            ref_day = _story_reference_day(evt_day_name, evt_numDay)
            return _story_num_day() >= _story_to_int(ref_day, 0) + _story_to_int(delta, 1)
        ...
        def checkReqs(self):
            ...
            for stat, limit in dict(self.reqs).items():
                ...
                if stat in peopleInfo:
                    ...
                    current_value = _story_to_int(globals().get(stat, 0), 0)
        ...
        def checkItem(self):
            if not self.item:
                return True
            inventory = globals().get("inventory", None)
...
init python:
    ...
    def _story_take_amanda_jump(default_target=""):
        ...
        next_label = str(globals().get("AmandaDynamicNextJump", "") or "")
    ...
    def story_amanda_prepare_entry(location_name):
        ...
        check_func = globals().get("CheckIfSexEventExist")
        if not callable(check_func):
            return False

        cur_time = _story_to_int(globals().get("time", 0), 0)
        chance_to_notice = 5
    ...
    def story_amanda_run_to_legare(location_name):
        ...
        get_func = globals().get("GetSexEventFromTable")
        let_go_func = globals().get("LegareAmandaLetGoCode")
        yell_func = globals().get("AmandaYellNotWork")
