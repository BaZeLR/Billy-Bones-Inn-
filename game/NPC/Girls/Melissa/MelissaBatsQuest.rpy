...
label MelissaBatBreakfastScene:
    ...
    if renpy.loadable("images/kitchen/need_kitty_1.png"):
        call ShowImage("", "", "images/kitchen/need_kitty_1.png")
    ...
    if renpy.loadable("images/melissa/yawns.png"):
        call ShowImage("", "", "images/melissa/yawns.png")
    ...
    if renpy.loadable("images/melissa/kitchen.jpg"):
        call ShowImage("", "", "images/melissa/kitchen.jpg")
    ...
    if renpy.loadable("images/kitchen/need_kitty_2.png"):
        call ShowImage("", "", "images/kitchen/need_kitty_2.png")
    ...
    "Разговор за столом быстро становится серьезнее. У вас в голове остается одна ясная мысль: с комнатой Мелиссы придется разбираться всерьез."
    $ MainTxt = "Утренний стол уже накрыт, но одного места не хватает. Аманда первой замечает пустую скамью Мелиссы и с ленивой усмешкой тянет: \"Вот увидите, сейчас она явится с таким лицом, будто всю ночь воевала с нечистой силой.\"\n\nВ этот момент в кухню, зевая и еле переставляя ноги, входит Мелисса. Вид у нее злой и невыспавшийся. Мелисса садится за стол и, даже взяв кружку, продолжает коситься так, будто над ее головой все еще что-то шуршит.\n\nСандра уже спокойнее говорит: \"У нас уже была крысиная проблема, из-за которой портились припасы, а теперь еще и летучие мыши? После крыс в кладовой я не хочу ждать, пока новая дрянь опять испортит дом.\"\n\nАманда тут же оживляется, складывает пальцы в дразнящий знак и тянет с ухмылкой: \"Мелисса, а что если ты настоящая ведьма? Крысы в подвале, мыши с крыльями под крышей... Может, это все твои любимцы сбежались?\"\n\nМелисса зло щурится: \"Если я ведьма, то первым делом заколдую кое-кому язык, чтобы он хоть за завтраком помолчал.\"\n\nРазговор за столом быстро становится серьезнее. У вас в голове остается одна ясная мысль: с комнатой Мелиссы придется разбираться всерьез."
    $ CurLocDesc = MainTxt
...
label MelissaNightNoiseScene:
    ...
    if str(_layout_last_picture or "").strip():
        call ShowImage("", "", _layout_last_picture)
