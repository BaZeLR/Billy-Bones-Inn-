...
init python:
    ...
    def melissa_private_context_active(room_code=""):
        ...
        return (
            _melissa_int(Melissa.var.get("private_context_day", -1), -1) == _melissa_int(dayspassed, 0)
    ...
    def melissa_start_scene_count():
        if int(Melissa.var.get("StartDay", -1) or -1) != int(dayspassed or 0):
            return 0
...
label IntMelissaTalkRefresh(girl_name="melissa"):
    ...
    if melissa_storage_thanks_available():
        $ current_action_items.append(MenuItem("Послушать, что Мелисса скажет о кладовой", Call("IntMelissaTalkApply", girl_name, "storage_thanks")))
    ...
    if melissa_relationship_allows(girl_name, "intimacy") and melissa_room_is_private(CurLoc):
        $ current_action_items.append(MenuItem("Уединиться с Мелиссой", Call("IntMelissaSex", girl_name, CurLoc)))
    elif melissa_relationship_allows(girl_name, "intimacy") and bool(melissa_private_place_offer(girl_name, CurLoc).get("ok", False)):
        $ current_action_items.append(MenuItem("Найти укромное место с Мелиссой", Call("IntMelissaFindPrivatePlace", girl_name, CurLoc)))
    elif melissa_relationship_allows(girl_name, "start") and int(Melissa.var.get("StartDay", -1) or -1) != int(dayspassed or 0):
        $ current_action_items.append(MenuItem("Сблизиться с Мелиссой", Call("IntMelissaStartMenu", girl_name)))
    if str(CurLoc or "") == "TavernMain" and str(getLocation("clara") or "") == "TavernMain" and int(Melissa.var.get("AskedAboutClaraDay", -1) or -1) != int(dayspassed or 0) and int(Melissa.asked_today or 0) == 0:
        $ current_action_items.append(MenuItem("Спросить Мелиссу о Клариссе", Call("IntMelissaTalkApply", girl_name, "ask_clara")))
    if int(Melissa.asked_today or 0) == 0 and household_special_talk_available(girl_name):
        $ _melissa_special_entry = household_special_talk_entry(girl_name)
        if _melissa_special_entry is not None:
            $ current_action_items.append(MenuItem(str(_melissa_special_entry.get("label", "Спросить о чем-то важном") or "Спросить о чем-то важном"), Call("IntMelissaTalkApply", girl_name, "insight")))

    if int(Melissa.talked_today or 0) < 3 and int(Melissa.rel or 0) < 5:
        $ current_action_items.append(MenuItem("Попробовать помириться с Мелиссой", Call("IntMelissaTalkApply", girl_name, "reconcile")))

    if int(Melissa.rel or 0) > 8 and CheckDailyEventExists("", "BuyDressTom") == 0 and CheckDailyEventExists(girl_name, "BuyDress") == 0 and int(Melissa.talked_today or 0) < 2 and week != 6:
        $ current_action_items.append(MenuItem("Предложить купить Мелиссе обновку", Call("IntMelissaTalkApply", girl_name, "buy_dress")))
    if int(Melissa.asked_today or 0) == 0 and int(Melissa.rel or 0) >= 15:
        $ current_action_items.append(MenuItem("Спросить, что для нее сейчас важнее всего", Call("IntMelissaTalkApply", girl_name, "priorities")))
...
label IntMelissaTalkApply(girl_name="melissa", choice_code=""):
    ...
    if str(choice_code or "") == "ask_clara":
        $ Melissa.mark_asked()
        $ Melissa.var["AskedAboutClaraDay"] = int(dayspassed or 0)
    ...
    if str(choice_code or "") == "storage_thanks":
        $ Melissa.var["StorageThanksDay"] = int(dayspassed or 0)
        $ Melissa.change_social(friend_delta=1)
    ...
    if str(choice_code or "") == "attic_findings":
        $ Melissa.var["AtticFindingsDay"] = int(dayspassed or 0)
        $ Melissa.mark_asked()
...
label IntMelissaStartMenu(girl_name="melissa"):
    ...
    if melissa_start_scene_remaining() <= 0:
        $ current_action_items.append(MenuItem("На сегодня хватит", Call("IntMelissaTalkRefresh", girl_name)))
        return
    ...
    $ current_action_items.append(MenuItem("Остановиться на сегодня", Call("IntMelissaTalkRefresh", girl_name)))
    return
...
label IntMelissaStartApply(girl_name="melissa", start_action=""):
    ...
    $ Melissa.var["StartDay"] = int(dayspassed or 0)
...
label IntMelissaFindPrivatePlace(girl_name="melissa", source_room=""):
    ...
    if not bool(_melissa_private_offer.get("ok", False)):
        ...
        call IntMelissaTalkRefresh(girl_name)
        return
    $ Melissa.var["private_context_day"] = int(dayspassed or 0)
...
label IntMelissaRoomProblemAdviceMenu(girl_name="melissa"):
    ...
    if _holes_seen > 0 and _temp_room == "" and _stage < 4:
        $ current_action_items.append(MenuItem("Предложить пока ночевать у вас", Call("IntMelissaRoomProblemAdviceApply", girl_name, "mc_room")))
        $ current_action_items.append(MenuItem("Предложить перебраться к Аманде", Call("IntMelissaRoomProblemAdviceApply", girl_name, "amanda_room")))
        $ current_action_items.append(MenuItem("Предложить занять пустую комнату", Call("IntMelissaRoomProblemAdviceApply", girl_name, "empty_room")))
    ...
    $ current_action_items.append(MenuItem("Назад", Call("IntMelissaTalkRefresh", girl_name)))
    $ CurLocDesc = MainTxt
    return
...
label IntMelissaRoomProblemAdviceApply(girl_name="melissa", advice_code=""):
    ...
    if _melissa_advice == "mc_room":
        ...
        $ Melissa.var["bat_attic_check_day"] = max(int(Melissa.var.get("bat_attic_check_day", -1) or -1), int(dayspassed or 0) + 1)
