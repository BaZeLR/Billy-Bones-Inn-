label IntGeorgettTalkRefresh(girl_name="georgett", girl_loc="street"):
    ...
    if str(choice_code or "") == "ask_kids":
        ...
        $ current_action_items = georgett_talk_action_items(girl_name, girl_loc)
        else:
    ...
    if str(choice_code or "") == "invite_tavern":
        ...
        $ player.tavern_management.household_members = int(player.tavern_management.household_members) + 2 + int(_kids_get("ProstitutesKids", 0) or 0)
        $ Georgett.finish_talk()
...
label IntGeorgettTalkApply(girl_name="georgett", girl_loc="street", choice_code=""):
    ...
    if str(choice_code or "") == "ask_eddie_visit":
        ...
        $ Georgett.finish_talk()
        $ CurLocDesc = MainTxt
        return    $ Liza.sync_shared_state()label IntGeorgettDress(girl_name="georgett", girl_loc="street"):
    call IntGeorgettDressChange(girl_name)
    returnlabel IntGeorgettDress(girl_name="georgett", girl_loc="street"):
    call IntGeorgettDressChange(girl_name)
    returnlabel IntGeorgettTalkRefresh(girl_name="georgett", girl_loc="street"):
    $ main_ui_begin_talk_state("Разговор с Жоржеттой", girl_name)
    $ current_action_title = "Разговор с Жоржеттой"
    $ current_action_content = None
    $ current_action_items = []
...
init python:
    def georgett_grope_outcome(girl_name="georgett", girl_loc="street"):
        ...
        if not has_paid_context:
            ...
            jump StreetTavern {"text": grope_text, "show_current_sex": False}
    ...
    def georgett_talk_action_items(girl_name="georgett", girl_loc="street"):
        items = [
            MenuItem("Осмотреть", Function(NpcActionLookState, girl_name, CurLoc)),
            MenuItem("Болтать", Call("IntGeorgettTalkApply", girl_name, girl_loc, "smalltalk")),
        ]
        topic_captions = (
            ("clients", "Спросить о клиентах", "ask_clients"),
            ("sex", "Спросить о сексе", "ask_sex"),
            ("family", "Спросить о семье", "ask_family"),
            ("pregnancy", "Спросить о беременности", "ask_pregnancy"),
            ("kids", "Спросить о детях", "ask_kids"),
            ("gerhard", "Спросить об отце Герхарде", "ask_gerhard"),
        )
        for topic_key, caption, choice_code in topic_captions:
            if Georgett.can_ask_topic(topic_key):
                items.append(MenuItem(caption, Call("IntGeorgettTalkApply", girl_name, girl_loc, choice_code)))
        if int(Liza.story_value("SawChurchAfterCermon", 0) or 0) and Georgett.can_talk_today():
            items.append(MenuItem("Рассказать про Лизетту и отца Герхарда", Call("IntGeorgettTalkApply", girl_name, girl_loc, "tell_liza_gerhard")))
        if Georgett.can_invite_to_tavern() and Georgett.can_talk_today():
            items.append(MenuItem("Предложить работать у себя в трактире", Call("IntGeorgettTalkApply", girl_name, girl_loc, "invite_tavern")))
        if Georgett.can_work_tavern() and Georgett.can_talk_today():
            items.append(MenuItem("Спросить как работается у вас в трактире", Call("IntGeorgettTalkApply", girl_name, girl_loc, "ask_work")))
        if Georgett.can_work_tavern() and Georgett.can_talk_today() and int(Liza.story_value("GloryHoleAsked", 0) or 0) == 1 and int(Georgett.story_value("GloryHoleExplained", 0) or 0) == 0:
            items.append(MenuItem('Спросить про работу в "Пьяном Пирате"', Call("IntGeorgettTalkApply", girl_name, girl_loc, "ask_pirate")))
        if Georgett.can_work_tavern() and Georgett.can_talk_today() and player.tavern_management.glory_hole == 2 and int(Georgett.story_value("GloryHoleAgreed", 0) or 0) == 0:
            items.append(MenuItem("Договориться об условиях работы у глорихола", Call("IntGeorgettTalkApply", girl_name, girl_loc, "gloryhole_terms")))
        eddie_topic_ready = (
            (int(Georgett.story_value("TellAboutEddieMomSex", 0) or 0) == 0 and (Becky.var.get("EddieTryToFuck", 0) == 4 or Becky.var.get("visitedhome", 0) >= 7))
            or (Becky.var.get("EddieGeorg", 0) == 0 and Eddie.var.get("TalkedAboutGeorgett", 0) == 1 and Becky.var.get("visitedhome", 0) >= 3 and (Eddie.var.get("SawMomSex", 0) > 0 or Becky.var.get("HomeSex", 0) > 0))
        )
        if eddie_topic_ready and Georgett.can_talk_today():
            items.append(MenuItem("Обсудить Эдди", Call("IntGeorgettTalkApply", girl_name, girl_loc, "talk_eddie")))
        if Becky.var.get("EddieGeorg", 0) > 0 and Becky.var.get("EddieWhoreHome", 0) == 0 and (player.economy.money > 25 or (Becky.var.get("EddieGeorg", 0) > 1 and player.economy.money > 10)) and Georgett.talk_count() < 2:
            items.append(MenuItem("Предложить Жоржетте проспонсировать ее визит к Эдди домой", Call("IntGeorgettTalkApply", girl_name, girl_loc, "sponsor_eddie_home")))
        if Becky.var.get("EddieGeorg", 0) > 0 and int(calendar_v2.hour or 0) <= 15:
            items.append(MenuItem("Спросить, не приходил ли Эдди", Call("IntGeorgettTalkApply", girl_name, girl_loc, "ask_eddie_visit")))
        if (player.economy.money >= 8 or (player.economy.money >= 4 and girl_loc == "tavern")) and Georgett.can_player_cum():
            items.append(MenuItem("Снять", Call("IntGeorgettTalkApply", girl_name, girl_loc, "hire")))
        items.append(MenuItem("Лапать", Call("IntGeorgettTalkApply", girl_name, girl_loc, "grope")))
        if Georgett.can_talk_today() and int(Georgett.rel or 0) >= 8 and int(Georgett.stats.get("pregnancy", 0) or 0) >= 120:
            dad_phrase = DaddyAskBuildPhrase(girl_name)
            if str(dad_phrase or "") != "":
                items.append(MenuItem("Поинтересоваться, знает ли она от кого залетела", Call("IntGeorgettTalkApply", girl_name, girl_loc, "ask_dad")))
        if girl_loc == "tavern":
            items.append(MenuItem("Обсудить одежду", Call("IntGeorgettTalkApply", girl_name, girl_loc, "dress")))
...
label IntGeorgettTalk(girl_name="georgett", girl_loc=""):
    if not girl_loc:
        $ girl_loc = "tavern" if str(CurLoc or "") == "TavernMain" else "street"
    if girl_loc == "street" and int(Georgett.rel or 0) <= 0:
    ...
    while True:
        menu:
            ...
            "Рассказать про Лизетту и отца Герхарда" if int(Liza.story_value("SawChurchAfterCermon",0) or 0) and Georgett.can_talk_today():
            ...
            "Спросить про работу в Пьяном Пирате" if Georgett.can_work_tavern() and Georgett.can_talk_today() and int(Liza.story_value("GloryHoleAsked",0) or 0)==1 and int(Georgett.story_value("GloryHoleExplained",0) or 0)==0:
                call IntGeorgettAskPirate(girl_name, girl_loc)
            "Договориться об условиях работы у глорихола" if Georgett.can_work_tavern() and Georgett.can_talk_today() and TavernGloryHole==2 and int(Georgett.story_value("GloryHoleAgreed",0) or 0)==0:
                call IntGeorgettGloryholeTerms(girl_name, girl_loc)
            "Обсудить Эдди" if Georgett.can_talk_today() and ((int(Georgett.story_value("TellAboutEddieMomSex",0) or 0)==0 and (Becky.var.get("EddieTryToFuck",0)==4 or Becky.var.get("visitedhome",0)>=7)) or (Becky.var.get("EddieGeorg",0)==0 and Eddie.var.get("TalkedAboutGeorgett",0)==1 and Becky.var.get("visitedhome",0)>=3 and (Eddie.var.get("SawMomSex",0)>0 or Becky.var.get("HomeSex",0)>0))):
                call IntGeorgettTalkEddie(girl_name, girl_loc)
            "Предложить Жоржетте проспонсировать ее визит к Эдди домой" if Becky.var.get("EddieGeorg",0)>0 and Becky.var.get("EddieWhoreHome",0)==0 and (player.economy.money>25 or (Becky.var.get("EddieGeorg",0)>1 and player.economy.money>10)) and Georgett.talk_count()<2:
                call IntGeorgettSponsorEddieHome(girl_name, girl_loc)
            "Спросить, не приходил ли Эдди" if Becky.var.get("EddieGeorg",0)>0 and int(calendar_v2.hour or 0)<=15:
                call IntGeorgettAskEddieVisit(girl_name, girl_loc)
            "Снять" if (player.economy.money>=8 or (player.economy.money>=4 and girl_loc=="tavern")) and Georgett.can_player_cum():
            ...
            "Обсудить одежду" if girl_loc=="tavern:
                call IntGeorgettDress(girl_name, girl_loc)
            "Закончить разговор":
                jump PortStreets
