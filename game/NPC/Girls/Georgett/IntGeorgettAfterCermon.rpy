...
label story_georgett_church_after_sermon_look_1:
    ...
    $ scene_runtime.text = "Посмотрев в замочную скважину вы увидели Жоржетту и отца Герхарда."
...
label story_georgett_church_after_sermon_look_2:
    show screen main_ui
    $ scene_runtime.text = "Вы продолжаете наблюдать за Жоржеттой и отцом Герхардом через замочную скважину."
    if Georgett.story_value("SawChurchAfterCermon", 0) == 0:
...
label story_georgett_church_after_sermon_look_3:
    ...
    if Georgett.cum_state("cum_inside_you") or Georgett.cum_state("cum_inside_others"):
...
label story_georgett_church_after_sermon_look_4:
    ...
    if Georgett.story_value("TalkChurchAfterCermonLiza", 0):
        ...
        if int(Liza.sex_stat("kids", 0) or 0) == 0:
