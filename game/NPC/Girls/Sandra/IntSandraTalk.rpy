...
label IntSandraReconcile(girl_name="sandra"):
    $ scene_runtime.text = "Вы подошли к маме и извинились за то, что были к ней несколько невнимательны и грубы последнее время. В свое оправдание вы заметили, что уберечь трактир от разорения очень сложно и вы все должны дружно работать вместе, чтобы преуспеть."
    if procedural_randint(1, 2, key="procedural:NPC/Girls/Sandra/IntSandraTalk.rpy:procedural_randint:40:1") == 1:
        $ scene_runtime.text += "\n\nМама благосклонно выслушала вас, обняла и сказала, что она всегда будет вас любить, несмотря ни на что!"
        call SlutFriendsIncrease(girl_name, 6, 1, 1, 0, 0, 0)
    else:
        $ scene_runtime.text += "\n\nМама холодно выслушала вас, отвернулась и пошла прочь, не говоря ни слова."
    $ Sandra.mark_talked()
    $ scene_runtime.location_text = scene_runtime.text
    $ main_ui_runtime.action_title = "Разговор с Сандрой"
    $ main_ui_runtime.action_content = None
