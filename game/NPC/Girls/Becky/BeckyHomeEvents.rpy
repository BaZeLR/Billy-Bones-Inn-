...
label story_becky_home_front_inga_0:
    ...
    $ Becky.var["HomeFrontCheckedDay"] = int(current_game_day() or 0)
    $ BeckyHomeFrontRoom.state["inga_scene_roll"] = procedural_randint(1, 4, "becky_home_front_%s" % int(current_game_day() or 0))
    if Becky.var.get("TodayFrontSexCheck", 0) == 1 and BeckyHomeFrontRoom.state["inga_scene_roll"] <= 2:
        $ BeckyHomeFrontRoom.state["inga_scene_roll"] = 3
    $ Becky.var["TodayFrontSexCheck"] = 1
    $ BeckyHomeFrontRoom.state["inga_scene_stage"] = 0
    ...
    $ pregnancy.setdefault("inga", 0)
    $ pregnancy.setdefault("inga", 0)
            if BeckyHomeFrontRoom.state["inga_scene_roll"] == 1:
        $ pregnancy_check("inga", "mouthface", 1, "Лукас")
    elif BeckyHomeFrontRoom.state["inga_scene_roll"] == 2:
        $ pregnancy_check("inga", "inside", 1, "Лукас")

    if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances":
    ...
    if BeckyHomeFrontRoom.state["inga_scene_roll"] <= 3:
        "Вдруг какое-то движение в темном углу за крыльцом привлекло ваше внимание."

    menu:
        "Зайти в дом":
            call BeckyHome(BeckyHomeFrontRoom.state["arrival_mode"])
            return True

        "Вернуться к трактиру" if BeckyHomeFrontRoom.state["arrival_mode"] == "guest":
            jump StreetTavern

        "Осторожно заглянуть за угол" if BeckyHomeFrontRoom.state["inga_scene_stage"] == 0:
            jump story_becky_home_front_peek_0

        "Сделать вид, что ничего там нет":
            $ BeckyHomeFrontRoom.state["inga_scene_stage"] = 10
...
label story_becky_home_front_peek_0:
    ...
    if BeckyHomeFrontRoom.state["inga_scene_roll"] == 1:
        ...
        $ _layout_last_picture = scene_image
        vscene scene_image
        $ BeckyHomeFrontRoom.state["inga_scene_stage"] = 1
        ...
        $ scene_image = "images/inga/StreetSex/pregfuckyou.jpg" if Inga.pregnancy_days() >= 120 else "images/inga/StreetSex/fuckyou1.jpg"
        $ _layout_last_picture = scene_image
        vscene scene_image
        $ BeckyHomeFrontRoom.state["inga_scene_stage"] = 1
        $ Inga.set_var_int("SawLucassex", 1)
        ...
        $ BeckyHomeFrontRoom.state["inga_scene_stage"] = 10
        return True
    if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances" and BeckyHomeFrontRoom.state["inga_scene_roll"] <= 2:
        "Ваши наблюдения заинтересовали Бекки, которая решила проверить, на что это вы там смотрите."

    menu:
        "Поделиться с вдовой своим открытием" if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances":
            jump story_becky_home_front_share_0

        "Сделать вид, что ничего там нет" if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances":
            jump story_becky_home_front_ignore_0

        "Предложить подойти к парочке" if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances":
            jump story_becky_home_front_suggest_approach_0

        "Подойти к парочке" if BeckyHomeFrontRoom.state["arrival_mode"] != "FromDances":
...
label story_becky_home_front_share_0:
    ...
    $ Inga.set_story_value_min("Knowher", 1)
    $ Inga.set_var_int("SawLucassex", 1)
    $ BeckyHomeFrontRoom.state["inga_scene_stage"] = 2
    menu:
...
label story_becky_home_front_suggest_approach_0:
    ...
    if Becky.var.get("visitedhome", 0) < 5:
        ...
        $ BeckyHomeFrontRoom.state["inga_scene_stage"] = 3
...
label story_becky_home_front_watch_0:
    ...
    if BeckyHomeFrontRoom.state["inga_scene_roll"] == 1:
        ...
        $ scene_image = "images/inga/StreetSex/cumfaceshow1.jpg" if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances" else "images/inga/StreetSex/cumface.jpg"
    else:
        ...
        if Inga.pregnancy_days() >= 120:
            $ scene_image = "images/inga/StreetSex/pregfuckshow.jpg" if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances" else "images/inga/StreetSex/pregfuckyou.jpg"
        else:
            $ scene_image = "images/inga/StreetSex/fuckshow1.jpg" if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances" else "images/inga/StreetSex/fuckyou1.jpg"
    $ _layout_last_picture = scene_image
    vscene scene_image
    ...
    if procedural_randint(1, 2, "becky_home_front_inga_greet_%s" % int(current_game_day() or 0)) == 1 and BeckyHomeFrontRoom.state["inga_scene_roll"] == 2 and Becky.var.get("IngaSexGreet", 0) == 0 and BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances":
    ...
    $ BeckyHomeFrontRoom.state["inga_scene_stage"] = 10
    menu:
        "Зайти в дом" if BeckyHomeFrontRoom.state["arrival_mode"] == "FromDances":
            call BeckyHome(BeckyHomeFrontRoom.state["arrival_mode"])
            return True
...
label story_becky_home_front_approach_0:
    ...
    $ Inga.apply_social_chance(0, 0, 0, 45, 3, 1, "becky_home_front_approach")
    menu:
        "Посмотреть как они кончат" if BeckyHomeFrontRoom.state["inga_scene_stage"] == 3:
            jump story_becky_home_front_watch_0
...
label story_becky_home_visit_0:
    ...
    show screen main_ui
    $ Becky.ensure_story_defaults()
    $ Becky.var["HomeEnterCheckedDay"] = int(current_game_day() or 0)
    $ BeckyAdmit = 0
    $ GirlName = "becky"
    $ Inga.ensure_story_defaults()
    $ pregnancy.setdefault("inga", 0)
    ...
    if BeckyAdmit == 1:
        ...
        $ _layout_last_picture = scene_image
        vscene scene_image
        if procedural_randint(1, 5, "becky_home_dinner_inga_%s" % int(current_game_day() or 0)) == 1:
...
label story_becky_home_from_dances_0:
    ...
    $ BeckyHomeFrontRoom.state["arrival_mode"] = ""
