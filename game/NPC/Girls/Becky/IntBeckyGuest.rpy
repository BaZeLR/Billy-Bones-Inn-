                ...
                if dinnerbecky == 0:
                    ...
                    if dinnerbeckyorgasm == 1:
                        ...
                        if procedural_randint(1, 4, "becky_dinner_eddie_notice_%s_%s" % (int(current_game_day() or 0), dinnerbecky)) == 1:
                            "Но все-таки ваши игры не остались незамеченными. Эдди пристально посмотрел на хозяйку и, как бы нечаянно, уронил нож и сразу нырнул за ним под стол. Вы убрали руку, но вылезший из-под стола Эдди расплылся в улыбке, а Бекки покраснела еще больше."
            ...
            "Взять Бекки под руку и идти наверх в спальню" if dinnertime == 6 and georgedinnersex == 0:
                ...
                if int(threads["beckyDinner"].num or 0) < 1:
                    ...
                    if (Eddie.ridiculed_follow_attempt and procedural_randint(1, 10, "becky_dinner_ridicule_repeat_%s" % int(current_game_day() or 0)) == 1) or (not Eddie.ridiculed_follow_attempt and procedural_randint(1, 2, "becky_dinner_ridicule_first_%s" % int(current_game_day() or 0)) == 1):
                        "Однако в этот раз к вам попытался присоединиться Беккин управляющий Эдди."
            ...
            "Идти в спальню вместе с Бекки и Эдди" if dinnertime == 6 and georgedinnersex == 0 and threads["beckyEddieSex"].completed:
                ...
                if Becky.corruption >= 60:
                    ...
                    if Eddie.others_saw_with_mother:
                        ...
                        "Ингенборг открыв рот смотрела на Эдди, шествующего за Бекки в спальню и на ходу щипающего хозяйку за попу. Лукас же быстро посмотрел на эту картину, соориентировался в ситуации и начал заворачивать Инге юбку, пользуясь ее прострацией. Но тем временем вы уже поднялись на второй этаж и так и не узнали, чем дело у них закончилось."
