...
label IntBeckyGuest:
    ...
    while True:
        ...
        menu:
            ...
            "Выставить на стол вино и еду из вашего трактира" if dinnertime == 0 and player.tavern_management.winenum >= 30 and player.tavern_management.productnum >= 30 and Inga.var_int("Knowher", 0) >= 2:
            ...
            "Кушать" if dinnertime <= 5 and georgedinnersex == 0:
                ...
                if Becky.var.get("visitedhome", 0) == 4 and procedural_randint(1, 7, "becky_dinner_home5_%s_%s" % (int(current_game_day() or 0), dinnertime)) == 1:
                ...
                if Becky.var.get("visitedhome", 0) >= 5 and dinneringaminet == 0 and dinnertime <= 2 and procedural_randint(1, 6, "becky_dinner_inga_under_%s_%s" % (int(current_game_day() or 0), dinnertime)) == 1:
            ...
            "Взять Бекки под руку и идти наверх в спальню" if dinnertime == 6 and georgedinnersex == 0:
                ...
                if Becky.var.get("visitedhome", 0) < 4:
                    ...
                    if Becky.var.get("visitedhome", 0) >= 5 and (Becky.corruption + procedural_randint(1, 5, "becky_dinner_bed_gate_%s" % int(current_game_day() or 0)) + dinnerbeckyorgasm * 5 >= 48 or Becky.var.get("HomeSex", 0) == 1):
                        ...
                        if _kids_watch > 3 and procedural_randint(1, 2, "becky_dinner_eddie_georg_hint_%s" % int(current_game_day() or 0)) == 1 and Becky.var.get("EddieGeorg", 0) == 0:
                    ...
                    if (Eddie.var.get("RidiculeFollow", 0) > 0 and procedural_randint(1, 10, "becky_dinner_ridicule_repeat_%s" % int(current_game_day() or 0)) == 1) or (Eddie.var.get("RidiculeFollow", 0) == 0 and procedural_randint(1, 2, "becky_dinner_ridicule_first_%s" % int(current_game_day() or 0)) == 1):
