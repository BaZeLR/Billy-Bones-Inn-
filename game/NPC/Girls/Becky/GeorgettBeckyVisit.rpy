...
label GeorgettBeckyVisit:
    ...
    if Becky.eddie_georgett_stage == 1:
        ...
        "\"Госпожа Блэнкеншип, вы же сказали Инге, чтобы она не стеснялась и приводила своего Лукаса к нам, мол дело молодое? Я так понял, что это и ко мне относится. Вот подружка моя, Жоржетта,\" - скороговоркой протарабанил Эдди."
        ...
        "\"Ну и что, может мне как раз и нравятся женщины вашего возраста,\" - смотря хозяйке в глаза нагло ответил Эдди."
    ...
    if KidsWatch <= 3:
        $ BeckyGuestSexDesc += "Вдруг вы заметили, что дверь в столовую приоткрыта и за вами кто-то подсматривает. "
        if KidsWatch == 1:
            $ BeckyGuestSexDesc += "Это был Ивар, младший сын вдовы. Он, не отрываясь, смотрел, как отсасывают его мама и старшая сестра, а его рука залезла в штаны. "
        elif KidsWatch == 2:
            $ BeckyGuestSexDesc += "Это была юная Эмма, средняя дочка Бекки. Она, кажется, наблюдает за вашей оргией с интересом, восхищением и возбуждением. "
        else:
            $ BeckyGuestSexDesc += "Это была Эмма с маленькой Лаурой, младшей дочкой Бекки. Лаура явно поражена происходящим, ее глазенки расширились, а щеки залила краска. Эмма же, на правах старшей, наклонилась к ней и шепотом комментирует происходящее. "

    if procedural_randint(1, 2, "becky_georgett_eddie_finish_%s" % int(current_game_day() or 0)) == 1:
    ...
    if procedural_randint(1, 2, "becky_georgett_lucas_finish_%s" % int(current_game_day() or 0)) == 1:
            ...
            "Расстегнуть штаны и позвать Бекки" if georgedinnersex == 1 and player.intimacy.came_today < player.intimacy.can_cum_daily:
                ...
                if Becky.corruption <= 55:
                    ...
                    if beckyminetagree > 1:
                        ...
                        "Млея от наслаждения, вы вдруг заметили полный похоти взгляд, которым Эдди одарил свою хозяйку. Впрочем, вскоре вы выкинули это из головы, так как Бекки почти довела вас до разрядки."
                        $ scene_runtime.picture = "images/becky/dinner/BeckyMinet1.jpg"
                        vscene scene_runtime.picture
                        $ scene_runtime.picture = "images/becky/dinner/BeckyMinet2.jpg"
                        vscene scene_runtime.picture
                else:
                    "Вы поймали взгляд вдовы, направленный на ваши вздувшиеся бугром штаны. Облизав губы в предвкушении вкусняшки, Ребекка, не смущаясь присутствием своего управляющего и дочки, встала перед вами на колени, приспустила штаны и начала облизывать ваш поднявшийся орган, лаская себя правой рукой под подолом платья."
                    "Млея от наслаждения, вы вдруг заметили полный похоти взгляд, которым Эдди одарил свою хозяйку. Взгляд, который она ему вернула. Впрочем, вскоре вы выкинули это из головы, так как Бекки почти довела вас до разрядки."
                    $ scene_runtime.picture = "images/becky/dinner/BeckyMinet1.jpg"
                    vscene scene_runtime.picture
                    $ scene_runtime.picture = "images/becky/dinner/BeckyMinet2.jpg"
                    vscene scene_runtime.picture
