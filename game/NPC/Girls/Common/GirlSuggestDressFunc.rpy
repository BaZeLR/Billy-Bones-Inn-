                    ...
                    "Попросить разрешить вам остаться":
                        ...
                        if ShowOffLevel <= 0:
                            ...
                            if DressBuyIsRelative == 1:
                                '"А ты не забыл, с кем так разговариваешь? Нет? Вот и живи потом со своими словами!"'
...
label GirlDressBuyShowInside(GirlName="", DressToBuy="", ShowOffLevel=0, DressBuyIsRelative=0):
    ...
    if DressBuyIsRelative == 1:
        ...
        if GirlName == "amanda":
            '"А это ничего, что ты мой хозяин и покровитель?" как бы невзначай спросила она.'
        else:
            '"А это ничего, что ты мой благодетель и опора трактира?" как бы невзначай спросила она.'
...
label GirlDressBuyJerkoff(GirlName="", DressToBuy="", DressBuyIsRelative=0):
    ...
    if DressBuyIsRelative == 1:
        ...
        '"Ох, и не стыдно, благодетель ты мой, что у тебя на меня встал колом!" с напускной скромностью заметила [_rn].'
    else:
...
label GirlDressBuyShowOutside(GirlName="", DressToBuy="", ShowOffLevel=0, DressBuyIsRelative=0):
    ...
    if DressBuyIsRelative == 1:
        ...
        '"А моему покровителю Стефану только того и надо!"'
    else:
