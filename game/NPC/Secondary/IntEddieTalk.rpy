...
label IntEddieTalk(preserve_text=False):
    if str(CurLoc or "") == "GroceryStore":
        $ _eddie_talk_picture = str(grocery_store_talk_picture("eddie") or "").strip()
        if _eddie_talk_picture:
            call ShowImage("", "", _eddie_talk_picture)
    $ _eddie_name = "eddie"
    $ UI_mode = "talk"
    $ UI_selected_char = _eddie_name
    $ current_girl_key = _eddie_name
    ...
    if Friends.get(_eddie_name, 0) >= 3 and BeckyVar.get("HomeSex", 0) > 0 and EddieVar.get("SawMomSex", 0) > 0 and EddieVar.get("SawWithGeorgett", 0) > 0 and BeckyVar.get("EddieTryToFuck", 0) != 1 and Talked.get(_eddie_name, 0) < 2:
        $ current_action_items.append(MenuItem("Предложить помочь подкатится к хозяйке лавки.", Call("IntEddieTalkApply", "mom_helper")))
    ...
    $ current_action_items.append(MenuItem("Закончить разговор", Jump("GroceryStore")))
    return
