default player.combat.party = []            self.location = ""    def dog_sync_profile():
    ...
    if _dog_after_load_init not in config.after_load_callbacks:
        ...
        $ dog_sync_profile()        $ dog_sync_profile()    $ dog_sync_profile()    def dog_action_talk_state(room_code=""):
    ...
    def dog_main_ui_action_items(room_code="", include_card=True):
        ...
        if d.state_key == "stray":
            ...
            if int(d.last_play_day or -1) != int(calendar_v2.daysInGame or 0):
                items.append(MenuItem("Поиграть с псом", Call("IntDogTalkApply", room_key, "play")))
            if int(d.last_train_day or -1) != int(calendar_v2.daysInGame or 0):
                items.append(MenuItem("Позаниматься дрессировкой", Call("IntDogTalkApply", room_key, "train")))
            if player_has_bone():
                items.append(MenuItem("Угостить пса костью", Call("IntDogTalkApply", room_key, "bone")))
                items.append(MenuItem("Наградить костью за дрессировку", Call("IntDogTalkApply", room_key, "train_bone")))
    ...
    def ensure_dog_runtime():
        ...
        dog = DogCompanion()
        return dog
default player_company = []default player.combat.party = []            self.location = ""    def dog_sync_profile():
        d = ensure_dog_runtime()
        display_name = dog_display_name()
        dog_data = peopleData.get("dog", None) if isinstance(peopleData, dict) else None
        if dog_data is not None:
...
init -5 python:
    ...
    def dog_is_here(room_code=""):
        return dog.is_here(str(room_code or ""), int(dayspassed or 0), int(time or 0))
...
label IntDogTalk(room_code=""):
    ...
    while True:
        ...
        menu:
            ...
            "Поиграть с псом" if dog.state_key != "stray" and int(dog.last_play_day or -1) != int(dayspassed or 0):
                if dog.play():
            ...
            "Позаниматься дрессировкой" if dog.state_key != "stray" and int(dog.last_train_day or -1) != int(dayspassed or 0):
                if dog.train():
            ...
            "Взять пса на охоту" if dog.state_key != "stray" and not dog.in_company:
            ...
            "Оставить сторожить дом" if dog.state_key != "stray" and (dog.in_company or (dog.booth_built and _dog_room.startswith("Backyard"))):
                $ dog.in_company = False
            ...
            "Попросить Сандру погулять с псом" if "sandra" in dog_household_walk_candidates(_dog_room):
                ...
                $ MainTxt = "%s охотно берет пса и идет с ним гулять. Оба возвращаются заметно довольнее, а пес смотрит на вас и на %s с еще большим доверием." % (_dog_name, _dog_name)
                $ CurLocDesc = MainTxt
