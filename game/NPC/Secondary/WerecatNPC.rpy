...
init -8 python:
    def werecat_is_living_with_household():
        adopted_count = int(WerecatVar.get("adopted_count", 0) or 0)
        if int(WerecatVar.get("adopted", 0) or 0) == 1:
            adopted_count = max(1, adopted_count)
        return adopted_count >= 1 and int(WerecatVar.get("sold", 0) or 0) == 0
    ...
    def werecat_is_in_room(room_code=""):
        ...
        try:
            ...
            return str(CurrentLoc.get("werecat", "") or "") == room_key
    ...
    for _werecat_slot, _werecat_rooms in WERECAT_ROAM_ROOMS.items():
        for _werecat_room in _werecat_rooms:
            _werecat_schedule_entries.append(NPCScheduleEntry(
                ...
                awake=True,
                talkable=True,
                condition=npc_schedule_rule("werecat_roam_match", location=_werecat_room),
                priority=120,
                label="werecat_roam",
            ))
...
label IntWerecatTalkApply(room_code="", choice_code=""):
    ...
    if _werecat_choice == "milk":
        if werecat_consume_milk(room_code):
            $ WerecatNPCState["milk_day"] = int(dayspassed or 0)
            $ WerecatNPCState["trust"] = min(20, int(WerecatNPCState.get("trust", 0) or 0) + 2)
            $ WerecatNPCState["comfort"] = min(20, int(WerecatNPCState.get("comfort", 0) or 0) + 2)
            $ fun = _player_clamp(int(fun or 0) + 2, 0, 100)
    ...
    if _werecat_choice == "play":
        $ WerecatNPCState["play_day"] = int(dayspassed or 0)
        $ WerecatNPCState["trust"] = min(20, int(WerecatNPCState.get("trust", 0) or 0) + 1)
        $ WerecatNPCState["comfort"] = min(20, int(WerecatNPCState.get("comfort", 0) or 0) + 1)
