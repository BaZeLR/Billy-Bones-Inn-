...
init python:
    ...
    def tavern_main_build_description():
        ...
        if cursed_sofa_installed():
            desc_parts.append("У стены теперь стоит купленный у тайного торговца старинный диван. Иногда его резные ножки сами собой переступают по полу, а из обивки доносится приглушенное ворчание.")
