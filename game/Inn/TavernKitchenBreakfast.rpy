...
label TavernKitchenBreakfastMelissaAmandaGerhard:
    $ renpy.dynamic("_breakfast_story_lines", "_breakfast_story_line_index")
    if not tavern_breakfast_melissa_amanda_gerhard_ready():
        return
    vscene BREAKFAST_GIRLS_TEASE_PICTURE
    python:
        _breakfast_story_lines = [
            "Завтрак еще не успевает толком начаться, как Сандра снова заводит про кладовую.",
    ...
    $ _breakfast_story_line_index = 0
    while _breakfast_story_line_index < len(_breakfast_story_lines):
        $ scene_runtime.text = _breakfast_story_lines[_breakfast_story_line_index]
        $ scene_runtime.location_text = scene_runtime.text
        $ tavern_kitchen_set_saved_text(scene_runtime.text)
        menu:
            "Продолжить":
                $ _breakfast_story_line_index += 1
    menu:
...
label TavernKitchenBreakfastMelissaAmandaGerhardNatural:
    $ renpy.dynamic("_breakfast_response_lines", "_breakfast_response_line_index")
    if not tavern_breakfast_melissa_amanda_gerhard_ready():
        return
    ...
    python:
        _breakfast_response_lines = [
            "Вы обрываете спор и говорите, что сначала надо разобраться с настоящей грязью: крысы в кладовой, летучие мыши под крышей, щели на чердаке. Остальное за столом можно оставить для тех часов, когда дом не трещит по углам.",
    ...
    $ _breakfast_response_line_index = 0
    while _breakfast_response_line_index < len(_breakfast_response_lines):
        $ scene_runtime.text = _breakfast_response_lines[_breakfast_response_line_index]
        $ scene_runtime.location_text = scene_runtime.text
        $ tavern_kitchen_set_saved_text(scene_runtime.text)
        menu:
            "Продолжить":
                $ _breakfast_response_line_index += 1
    call stat
    return
