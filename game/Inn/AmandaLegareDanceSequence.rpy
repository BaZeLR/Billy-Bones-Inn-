...
label LegareAmandaGoMenu():
    ...
    menu:
        "Проследить за ними":
            jump after_dance_sex_legare
        "Продолжить танцевать":
            # Show message and run let-go code
            "Решив не вмешиваться в личную жизнь вашей сестры, вы проводили парочку взглядом и остались танцевать."
            call LegareAmandaLetGoCode
            jump FridayDance
        "Остановить их и отправить Аманду домой":
            call after_dance_legare_prohibit
            jump FridayDance
    return

# --- Amanda/Legare let-go outcome logic ---
label LegareAmandaLetGoCode(args=None):
    $ _legare_plan = build_legare_amanda_let_go_plan(*tuple(args or ()))
    $ AmandaVar["sucklegare"] = int(_legare_plan.get("sucklegare", 0) or 0)
    $ AmandaVar["fucklegare"] = int(_legare_plan.get("fucklegare", 0) or 0)
    $ AmandaVar["deflowerlegare"] = int(_legare_plan.get("deflowerlegare", 0) or 0)
    if _legare_plan.get("set_virginity", None) is not None:
        $ virginity["amanda"] = int(_legare_plan.get("set_virginity", 0) or 0)
    $ AmandaVar["alberfriends"] = int(AmandaVar.get("alberfriends", 0) or 0) + int(_legare_plan.get("alberfriends_delta", 0) or 0)
    if tuple(_legare_plan.get("slut_args", ()) or ()):
        $ _legare_slut_args = tuple(_legare_plan.get("slut_args", ()) or ())
        call SlutFriendsIncrease(_legare_slut_args[0], _legare_slut_args[1], _legare_slut_args[2], _legare_slut_args[3], _legare_slut_args[4], _legare_slut_args[5], _legare_slut_args[6])
    if str(_legare_plan.get("pregnancy_target", "") or "") != "":
        call PregnancyCheck("amanda", _legare_plan.get("pregnancy_target", ""), 1, "legare")
    return

# --- Helper labels for outcomes ---
label after_dance_sex_legare():
    # ...scene for following Amanda and Legare after dance...
    return

label after_dance_legare_prohibit():
    # ...scene for stopping Amanda and sending her home...
    return

label pregnancy_check(girl, where, count, partner):
    # ...pregnancy check logic...
    return
