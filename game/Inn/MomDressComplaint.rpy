...
label MomDressComplaint(girl_name):
    ...
    $ _mdc_georgett_present = str(CurrentLoc.get("georgett", "") or "") == "TavernMain"
    hide screen main_ui
    ...
    if sluttiness.get("sandra", 0) < 50:
        ...
        if _mdc_georgett_present:
            $ _mdc_intro_text = str(_mdc_intro_text or "") + "\n\nЭто все от Жоржи с Лизкой идет! Сыночек, я конечно понимаю, что от этих шлюх нам доходик капает, но ты посмотри, как они на нашу {}у влияют!".format(GirlSillyName)
