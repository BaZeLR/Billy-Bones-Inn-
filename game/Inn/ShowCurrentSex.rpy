    ...
    if Arousal.get("You", 0) >= 100:
        if CockInPussy.get(GirlNameSCS, 0) == 1:
            if GirlNameSCS == "liza" and sluttiness.get(GirlNameSCS, 0) < 50 and pregnancy.get(GirlNameSCS, 0) < 120:
                ...
                "\"Стефан, ты что, кончаешь?!\" как будто почуствовала [RealName.get(GirlNameSCS, GirlNameSCS)]. \"Только не в меня, я не хочу чтобы меня родной брат обрюхатил! Вытащи, хорошо?\""
            ...
            if EddieCockInPussy.get(GirlNameSCS, 0) == 1:
                if CockInMouth.get(GirlNameSCS, 0) == 1:
                    if sluttiness.get(GirlNameSCS, 0) < 65:
                        "[RealName.get(GirlNameSCS, GirlNameSCS)] почувствовала что ее сын готов вот-вот кончить и что-то с ужасом в голосе промычала, однако вы так и не вынули своего члена у нее изо рта и поэтому бедная вдова на смогла ясно выразить свою мысль."
                    else:
                        "[RealName.get(GirlNameSCS, GirlNameSCS)] почувствовала что ее сын готов вот-вот кончить и что-то промычала, но что именно было не понять, ваш член во рту мешал ей выражаться ясно и понятно."
                else:
                    if pregnancy.get(GirlNameSCS, 0) >= 120:
                        "Ребекка, почусвтвовав что ее сын уже близок к разрядке, просто сказала: \"Может кончать куда вздумается, более беременной чем сейчас ты меня все равно не сделаешь.\""
                    elif sluttiness.get(GirlNameSCS, 0) < 65:
                        "Ребекка, почусвтвовав что ее сын уже близок к разрядке, быстро забормотала: \"Сыночек, миленький, только не в меня, ты слышишь? Ведь ты мой сын.\""
                    else:
                        "Ребекка, почусвтвовав что ее сын уже близок к разрядке, страстно закричала: \"Да, кончи в меня, я хочу почувствовать как ты кончаешь!\""
            elif EddieCockInMouth.get(GirlNameSCS, 0) == 1:
                "[RealName.get(GirlNameSCS, GirlNameSCS)] чувствует что ее сын уже готов разрядиться и приглашающе мычит, продолжая сосать его член."
    ...
    if Arousal.get(GirlNameSCS, 0) >= 100:
        ...
        if GirlNameSCS == "amanda":
            if GiveOrgasms.get(GirlNameSCS, 0) == 4:
                "\"Ох, братишка, как же хорошо с тобой!\", довольно сказала вам [RealName.get(GirlNameSCS, GirlNameSCS)]. \"Ты такой ласковый, правильно я тебе дала!\""
                $ Friends[GirlNameSCS] = Friends.get(GirlNameSCS, 0) + 1
