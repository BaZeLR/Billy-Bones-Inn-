...
init python:
    ...
    def melissa_temp_room_text():
        ...
        if temp_room == "TavernEmptyRoom":
            if waiting_for_repair:
                return "Из-за летучих мышей и щелей под крышей Мелисса пока перебралась в гостевую комнату и ждет, пока закончится заказанная починка крыши."
            return "Из-за летучих мышей и паутины Мелисса пока присматривается к гостевой комнате, пока вы не решите проблему."
        return ""
    ...
    def household_barber_request_ready(girl_name="", request_context=""):
        ...
        if current_game_day() - int(household.barber_request_last_day.get(girl, -14) or -14) < 14:
            return False
        if girl in ("liza", "georgett"):
            return bool(girl_info is not None and girl_info.is_tavern_worker() and girl_info.social_action_allowed("gift"))
...
label HouseholdBarberRequestEvent(girl_name="", request_context="breakfast"):
    ...
    elif _barber_girl == "liza":
        $ scene_runtime.text = "За завтраком вы предлагаете Лизетте сходить к Серджио. Она поправляет волосы и кивает: \"Я бы сходила. Когда прическа аккуратная и сама выглядишь ухоженно, к тебе охотнее подходят. Для работы это полезнее, чем ходить растрепанной.\""
    elif _barber_girl == "georgett":
        $ scene_runtime.text = "За завтраком вы предлагаете Жоржетте визит к Серджио. Она усмехается: \"Дельная мысль. Клиент сперва глазами выбирает. Хорошая стрижка и ухоженный вид тоже на заработок работают, а не только настроение поднимают.\""
    $ scene_runtime.location_text = scene_runtime.text
    show screen main_ui
    menu:
