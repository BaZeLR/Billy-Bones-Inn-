init python:
    ...
    def DaddyAskBuildName(girl_name, dude_name, dude_name_type, times=0):
        ...
        if str(girl_name) == "amanda" and dude_name_l == "вы":
            rand_var = renpy.random.randint(1, 3)
            if rand_var == 1:
                return "ты братик, кто же еще? Завернул сестричке родной подол и обрюхатил меня. Думай теперь как мамке все объяснить"
            if rand_var == 2:
                return "не кто иной, как ты. Наспускал мне полную киску своего семени, вот у меня пузо и растет теперь"
            return "ты. Вот блин, от родного брата понесла, стыд-то какой!"
        ...
        if str(girl_name) == "becky" and dude_name_l == "эдди":
            rand_var = renpy.random.randint(1, 3)
            if rand_var == 1:
                return "сыночек мой старшенький ненаглядный. Тяжело ведь мне одной без мужика, вот и баловалась с ним. А теперь он мне и заделал ребеночка"
            if rand_var == 2:
                return "Эдди, сыночек мой. Вот и рожу сына и внука одновременно. Или дочку-внучку. Надеюсь не врал отец Герхарт, нет в этом большого греха"
            return "Эдди, родненький мой сын. Добаловались, докувыркались мы с ним"
        ...
        if "эдди" in dude_name_l:
            rand_var = renpy.random.randint(1, 3)
            if rand_var == 1:
                return "Эдди, сын вдовы Блэнкеншип, ну той у которой ты продукты покупаешь"
            if rand_var == 2:
                return "Эдди, ну знаешь ты его, он у свой матери в лавке работает"
