            ...
            if sluttiness.get("amanda", 0) >= 62:
                ...
                if pregnancy.get("amanda", 0) > 120:
                    ...
                    if sluttiness.get("amanda", 0) >= 57:
                        ...
                        if rand_var == 1:
                            "\"Цветочек он принес?!\" отвечает чуть смягчившиь ваша сестра, \"да иди ты со своим цветочком!\""
                            call amanda_lover_ask_cum_code
                        elif rand_var == 2:
                            "\"Нда, цветочек так себе,\" пробормотала ваша сестра. \"Ну да ладно, дорог не подарок, дорого внимание. Но могу только ротиком.\""
                            call amanda_lover_ask_minet_agree
                        else:
                            "\"Ну спасибо, хоть кто-то внимание проявил,\" отвечает ваша сестра.\nНда, не слишком-то целомудренно реагирует ваша сестра даже на самые немудренные ухаживания."
                            call amanda_lover_ask_cum_code
                    else:
                        call amanda_lover_ask_shy_code
...
label amanda_lover_show_sex_scene(scene_type, guy_name):
    ...
    if amanda_lover_build == 1:
        "[guy_name] и Аманда шли недолго, через пару поворотов галантный кавалер пропустил вашу сестру первой в какую-то калиточку а потом зашел туда сам. Легким шагом, делая вид что прогуливаетесь, вы подвалили к этой калиточке и дернули ее на себя."
    else:
    ...
    if amanda_lover_build_get_in == 1:
        ...
        if amanda_lover_build == 1:
            ...
            if scene_type == "minet":
                "[guy_name] стоит, прислонившись к телеге, а Аманда, стоит перед ним на коленях и отсасывает. Вдруг он попробовал было вытащить свой член из ее рта, но она протестуеще замычала, принимая весь заряд его семени в рот.\nЗакончив сглатывать, она подняла на него глаза и сказала: \"Но-но, головой надо думать иногда! Если бы твоя сперма попала мне на платье, то как бы потом я его отстирывала? Говорят, что в республике Атлантида именно так уличили в измене жене тамошнего дожа!\"\nНда, не ожидали вы от сестренки такой эрудиции."
                $ ShowImageSeq("amanda", "RandomSex", "minet", 5)
            else:
                "[guy_name] разложил вашу сестренку на телеге и невозбранно трахает ее. Той же, похоже, это нравиться. Впрочем, вы похоже застали уже самое окончание совокупления, если так можно выразиться."
            ...
            if scene_type == "minet":
                "[guy_name] развалился на куче сена, а Аманда, склонившись над ним, делала ему приятное. Собственно вы застали самый финал, когда ваша сестрица получила полный заряд прямо в рот. И что характерно, все проглотила."
                $ ShowImageSeq("amanda", "RandomSex", "minet", 5)
            else:
                "[guy_name] уложил вашу сестренку на куче сена и трахает ее, а та ему с энтузиазмом подмахивает. Причем похоже, что дело близится к концу. Так и есть!"
                $ ShowImage("amanda", "RandomSex", "sex")
