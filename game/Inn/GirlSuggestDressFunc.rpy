                    ...
                    "Попросить разрешить вам остаться":
                        ...
                        if ShowOffLevel <= 0:
                            ...
                            if DressBuyIsRelative == 1:
                                '"А ты не забыл, что я тебе мать? Нет? Вот и рожай таких!"'
                            elif DressBuyIsRelative == 2:
                                '"Так ты с сестрой родной поступаешь? Подонок!"'
                            else:
...
label GirlDressBuyWaitPay(GirlName="", DressToBuy=""):
    ...
    'К вашему разочарованию, благодарность [_rn2] этими словами и ограничилась: довольная девица быстрым шагом направилась к выходу, оставив вас расплачиваться.'
...
label GirlDressBuyShowInside(GirlName="", DressToBuy="", ShowOffLevel=0, DressBuyIsRelative=0):
    ...
    if DressBuyIsRelative == 1:
        '"А тебе не стыдно так на мать свою пялиться?" продолжила она, чуть помедлив. "Совсем нет, мамусик, я просто восхищаюсь, какая ты у меня красивая," ловко парировали вы.'
    elif DressBuyIsRelative == 2:
        '"А это ничего, что я твоя сестра?" как бы невзначай спросила она. "Конечно ничего, что же плохого в том, чтобы сестренкой моей, красавицей из красавиц, восхититься?"'
    ...
    if str(_gds_get_dict("panties").get(GirlName, "") or "") == "":
        ...
        if (int(_gds_get_dict("sluttiness").get(GirlName, 0) or 0) > 49 and int(_gds_get_dict("HadSex").get(GirlName, 0) or 0) > 0) or int(_gds_get_dict("sluttiness").get(GirlName, 0) or 0) > 62:
            ...
            if DressBuyIsRelative == 1:
                '"Вот оттуда ты и появился!" добавила ваша разбитная мамаша.'
        else:
    ...
    menu:
        "Продолжить смотреть":
            'Вы продолжили лицезреть Ирму, снимающую мерку с [_rn2]. Вскоре они закончили и [_rn] оделась. Получив заверение портнихи, что платье будет готово уже к завтрашнему утру, счастливая [_rn] удалилась, оставив вас расплачиваться.'
        ...
        "Подрочить на зрелище" if _gds_player_hadsex() > 5 and _gds_player_cum_today() < _gds_player_cum_cap():
            ...
            if ShowOffLevel < 3:
                if DressBuyIsRelative == 1:
                    '"Да ты что, Стефан, совсем с глузду съехал? Я тебе мать или кто?!" закричала с негодованием [_rn].'
                elif DressBuyIsRelative == 2:
                    '"Да ты охренел, Стефан, дрочить на собственную сестру?!" пристыдила вас [_rn].'
                else:
...
label GirlDressBuyJerkoff(GirlName="", DressToBuy="", DressBuyIsRelative=0):
    ...
    if DressBuyIsRelative == 1:
        '"Значит мамочка у тебя еще не стара, раз у тебя на меня встал! Ну и шалопай ты у меня, сыночек!" прокомментировала ваши действия [_rn].'
    elif DressBuyIsRelative == 2:
        '"Ох, и не стыдно, что у тебя на родную сестру встал колом!" с напускной скромностью заметила [_rn].'
    else:
...
label GirlDressBuyShowOutside(GirlName="", DressToBuy="", ShowOffLevel=0, DressBuyIsRelative=0):
    ...
    if DressBuyIsRelative == 1:
        ...
        '"А братику моему только того и надо!"'
    else:
