...
init python:
    ...
    def main_ui_call_label(label_name="", *label_args):
        ...
        if callable(has_label_fn) and not has_label_fn(label):
            return
        renpy.exports.call_in_new_context(label, *tuple(label_args or ()))
    ...
    def main_ui_restore_room_scene_state():
        ...
        store.UI_mode = "scene"
        store.UI_selected_char = ""
        store.current_girl_key = ""
        store.current_action_content = None

        room_obj = store.CurrentRoom
        room_code = str(getattr(room_obj, "code_name", "") or store.CurLoc or "").strip()
        store.current_action_title = "Действия в трактире" if room_code == "TavernMain" else "Действия"
        if room_code == "MarketPlace" and hasattr(store, "marketplace_action_items"):
            store.current_action_items = list(store.marketplace_action_items() or [])
        else:
            ...
            store.current_action_items = list(room_sections.get("movement", [])) + list(room_sections.get("actions", []))
    ...
    def main_ui_begin_talk_state(title="", selected_char=""):
        ...
        if char_key:
            store.UI_selected_char = char_key
            store.current_girl_key = char_key

        if str(title or "").strip():
            store.current_action_title = str(title)
    ...
    def tractir_after_load_restore_ui():
        ...
        try:
            if hasattr(store, "current_action_content"):
                store.current_action_content = None
            if hasattr(store, "current_action_items"):
        ...
        try:
            if getattr(store, "CurrentRoom", None) is None and "get_registered_room" in globals():
                restored_room = get_registered_room(str(getattr(store, "CurLoc", "") or ""))
                if restored_room is not None:
                    store.CurrentRoom = restored_room
        except Exception:
            pass
...
screen current_action_panel():
    ...
    elif str(UI_mode or "") in ("mc", "char", "dog", "fight"):
        null
...
screen main_ui():
    ...
    if CurLoc == "Intro":
        ...
        key "game_menu" action [Function(ensure_ui_translation_callable), ShowMenu("save")]
                        ...
                        vbox:
                            ...
                            hbox:
                                ...
                                vbox:
                                    ...
                                    text "month: [month_name]" size 20 xalign 0.0
                                    text "year: [year]" size 20 xalign 0.0
                                    null height 4
                                ...
                                vbox:
                                    ...
                                    textbutton "Время":
                                        ...
                                        yminimum 42
                                        text_size 20
                                        action [Hide("girl_card_overlay"), Hide("player_card_overlay"), Hide("tavern_report_card_overlay"), Hide("time_change_card_overlay"), Show("time_change_card_overlay", return_label="__hide__")]
                            ...
                            if _char_entries:
                                grid 3 3:
                                    ...
                                    for _entry_index, _entry in enumerate(_char_slots):
                                        if _entry:
                                            $ _npc_name = str(_entry.get("name", "") or "")
                                            $ _npc_id = str(_entry.get("id", "") or "")
                                            $ _npc_talk_label = str(_entry.get("talk_label", "") or "")
                                            $ _npc_talk_args = tuple(_entry.get("talk_args", ()) or ())
                                            $ _npc_examine_id = str(_entry.get("examine_id", _npc_id) or _npc_id)
                                            $ _npc_can_examine = bool(_entry.get("can_examine", True))
                                            $ _npc_auto_card = bool(_entry.get("auto_card", False))
                                            if _npc_id.lower() == "you":
                                                ...
                                                $ _npc_action = [
                                                    Hide("girl_card_overlay"),
                                                    Hide("player_card_overlay"),
                                                    Function(show_npc_action_main_ui_state, _npc_id, _npc_talk_label, _npc_talk_args, _npc_examine_id, CurLoc, _npc_name, _npc_can_examine),
                                                ]
