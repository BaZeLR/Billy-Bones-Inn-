...
label NextDay(retlocname, timepassed):
    ...
    if money == 0:
        "К сожалению, ваш трактир разорился, у вас не осталось денег и вы были вынужденны продать его за долги. Придется все начать сначала!"
    if tavernvisitors == 0:
        "К сожалению, ваш трактир обрел настолько дурную славу, что в него больше никто не ходит. Вы были вынужденны продать его. Придется все начать сначала!"
