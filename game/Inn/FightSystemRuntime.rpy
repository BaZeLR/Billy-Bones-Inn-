...
init -20 python:
    ...
    def fight_ensure_runtime():
        ...
        globals().setdefault("FightVictoryLoot", {})
    ...
    def fight_hunt_intro_text(enemy_id="", enemy_count=1, room_code=""):
        ...
        if str(enemy_id or "") == "white_wolf":
            return "В чаще почти бесшумно показывается белый волк. Он держится один и смотрит прямо на вас, будто оценивает противника."
    ...
    def fight_collect_victory_loot():
        ...
        globals()["FightVictoryLoot"] = dict(loot_rows or {})
    ...
    def forest_trap_can_place(room_code=""):
        fight_ensure_runtime()
        return bool(fight_can_hunt_here(room_code)) and int(_player_item_count_by_id("hunting_trap_001") or 0) > 0 and int(ForestTrapState.get("active", 0) or 0) == 0
    ...
    def forest_trap_set(room_code=""):
        ...
        globals()["ForestTrapState"] = {"active": 1, "room": room_key, "day": int(dayspassed or 0), "armed_count": 1}
