        ...
        menu friday_dance_menu:
            ...
            "Найти Аманду" if FridayDancesCount < 5 and DanceStep == 0 and AmandaVar.get('leftdances',0) == 0:
                ...
                if AmandaVar.get('EscapeUnnoticed',0) == 1:
                    $ result = "Вы попробовали найти Аманду, но к своему удивлению не смогли этого сделать. На площади ее не было. Вокруг площади тоже. Может она отправилась домой, а может ее этот хрен Легаре за собой уволок, а может еще что стряслось, но так или иначе вы упустили сестренку."
                    $ AmandaVar['leftdances'] = 1
                elif AmandaVar.get('albernowdances',0) == 1:
                    $ result = "Вы прошлись по площади, ища свою сестренку Аманду, и обнаружили ее c мессиром Легаре."
                    call ShowImage("amanda", "dance", "legare_step_0")
                    $ DanceStep = 1
                else:
                    $ result = "Вы прошлись по площади, ища свою сестренку Аманду, и нашли ее скромно стоящей около одной из колонн."
...
label CheckIfAmandaGoneDance:
    if GetDanceJustLeft("amanda", "legare", FridayDancesCount) > 0 or AmandaVar["LegareGo"] == 1:
        if renpy.random.randint(1, 2) == 1:
            $ AmandaVar["leftdances"] = 1
            "Неожиданно вы заметили, что ваша младшая сестренка торопиться куда-то прочь под ручку с мессиром Легаре."
