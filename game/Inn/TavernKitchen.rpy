...
label story_becky_sandra_kitchen_visit:
    ...
    if _becky_sandra_visit_stage == 0:
        $ scene_runtime.text = "Зайдя вечером на кухню, вы застаете Сандру и Бекки за негромким разговором у разделочного стола.\n\nБекки вздыхает: \"Лавка, дом, закупки — все на мне. Иногда кажется, что стоит на миг отпустить поводья, и все развалится.\"\n\nСандра отвечает: \"Если рядом надежные люди, они хозяйке не обуза. Только с ними надо говорить прямо, а не ждать, пока сами догадаются.\""
    elif _becky_sandra_visit_stage == 1:
        $ scene_runtime.text = "В следующий визит Бекки снова сидит с Сандрой на кухне.\n\n\"Эдди мне верен, дом и лавку без него уже трудно представить,\" признается Бекки. \"Только смотрит он на меня в последнее время совсем не как управляющий.\"\n\n\"Молчанием такие вещи не исправляют,\" замечает Сандра. \"Так можно и хорошего человека оттолкнуть, и весь дом извести недомолвками.\""
    else:
        $ scene_runtime.text = "Снова встретив Бекки у Сандры, вы слышите продолжение их разговора.\n\n\"Внимание Эдди меня и тревожит, и радует,\" признается Бекки. \"Я боюсь перестать быть для него хозяйкой. Но если буду только отказывать и делать вид, что ничего не замечаю, сама же разрушу мир в доме.\"\n\nСандра кивает: \"Вот об этом и надо поговорить без увиливаний.\""
    $ scene_runtime.location_text = scene_runtime.text
        ...
        menu:
            ...
            "Подать горячую медовую настойку" if int(player.item_count("libido_tincture_001") or 0) > 0:
                ...
                menu:
                    "Выслушать Сандру":
                        pass
                $ scene_runtime.text = "\"В здоровом доме нужды друг друга не замалчивают,\" спокойно говорит Сандра. \"Если хозяйка и управляющий честны друг с другом и счастливы, в семье меньше злости, связь между домочадцами крепче, работа идет лучше. Значит, и для хозяйства, и для торговли это только на пользу.\""
                $ scene_runtime.location_text = scene_runtime.text
                menu:
                    "Выслушать решение Бекки":
                        pass
