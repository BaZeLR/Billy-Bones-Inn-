init python:
    ...
    def amanda_dress_change_has_options(girl_name="amanda"):
        ...
        if Talked.get(girl, 0) >= 2:
            return False
        if Friends.get(girl, 0) > 8 and GiveOrgasms.get(girl, 0) >= 2:
            return True
        if Friends.get(girl, 0) > 8 and CheckDailyEventExists("", "BuyDressTom") == 0 and CheckDailyEventExists(girl, "BuyDress") == 0 and week != 6:
            return True
        return False
    ...
    def amanda_dress_change_other_saw_text(girl_name="amanda", agreed_to_redress=0):
        ...
        if randvar == 1:
            if sluttiness.get("sandra", 0) >= 35:
                text = 'Обернувшись, вы вдруг встретились взглядом с вашей мамой, наблюдающей за этой сценкой. Но она всего лишь усмехнулась, покачала головой и пошла по своим делам.'
            else:
                text = 'Обернувшись, вы вдруг встретились взглядом с вашей мамой, наблюдающей за этой сценкой. И увиденное ей явно не понравилось. Она подскочила и сердито заорала: "Значит моя дочка становится шлюхой? Раздеваться при всех, ни стыда, ни совести!"\n"Мама, успокойся," заступились вы за сестру. У Аманды там просто что-то кололо, да так больно, что она и не сообразила где она."\nХоть и малоправдоподобная, но все-таки отмазка смутила вашу мать и вы смогли заболтать тему.'
        elif randvar == 2:
            if sluttiness.get("melissa", 0) >= 35:
                text = 'Вы окинули взглядом трактир и увидели пораженную Мелиссу, заметившую Амандин стриптиз. Но увиденное ее совсем не шокировало, она понимающе улыбнулась сестре и пошла дальше по своим делам.'
            else:
                text = 'Вы окинули взглядом трактир и увидели пораженную Мелиссу, заметившую Амандин стриптиз. И этот стриптиз ее явно шокировал. Она подбежала к вам и воскликнула: "Сестричка, ты что, совсем стыд потеряла?"\n"Знаешь, что, то что ты старшая, не дает тебе права кричать на меня," отвергла критику Аманда, и не слушая возмущенные писки Мелиссы, пошла как ни в чем ни бывало.'
        elif randvar == 3:
            ...
            text = ""
        if randvar <= 2:
            renpy.call("SlutFriendsIncrease", girl, 0, 0, 0, 60, 2, 1)
        if randvar >= 5 and randvar <= 7:
            renpy.call("SlutFriendsIncrease", girl, 0, 0, 0, 60, 1, 1)
            global tavernfame
...
label IntAmandaDressChangeRefresh(GirlNameIAT="amanda"):
    ...
    $ _can_offer_bra = Friends.get(GirlNameIAT, 0) > 8 and GiveOrgasms.get(GirlNameIAT, 0) >= 2 and str(bra.get(GirlNameIAT, "") or "") != "" and (AmandaVar.get("suckyou", 0) or AmandaVar.get("fuckyou", 0)) and Talked.get(GirlNameIAT, 0) < 2
    $ _can_offer_panties = Friends.get(GirlNameIAT, 0) > 8 and GiveOrgasms.get(GirlNameIAT, 0) >= 2 and str(panties.get(GirlNameIAT, "") or "") != "" and (AmandaVar.get("suckyou", 0) or AmandaVar.get("fuckyou", 0)) and Talked.get(GirlNameIAT, 0) < 2
    $ _can_shame = Friends.get(GirlNameIAT, 0) > 8 and GiveOrgasms.get(GirlNameIAT, 0) >= 2 and Talked.get(GirlNameIAT, 0) < 2
    $ _can_buy = Friends.get(GirlNameIAT, 0) > 8 and CheckDailyEventExists("", "BuyDressTom") == 0 and CheckDailyEventExists(GirlNameIAT, "BuyDress") == 0 and Talked.get(GirlNameIAT, 0) < 2 and week != 6

    if _can_offer_bra:
        $ current_action_items.append(MenuItem("Предложить сестренке ходить без лифчика", Call("IntAmandaDressChangeApply", GirlNameIAT, "offer_bra")))
    if _can_offer_panties:
        $ current_action_items.append(MenuItem("Предложить сестре снять панталоны", Call("IntAmandaDressChangeApply", GirlNameIAT, "offer_panties")))
    if _can_shame:
        $ current_action_items.append(MenuItem("Постыдить сестру за то, что ходит без лифчика", Call("IntAmandaDressChangeApply", GirlNameIAT, "shame_bra")))
        $ current_action_items.append(MenuItem("Постыдить сестру за отстутсвие панталон", Call("IntAmandaDressChangeApply", GirlNameIAT, "shame_panties")))
    if _can_buy:
        $ current_action_items.append(MenuItem("Предложить купить сестре обновку", Call("IntAmandaDressChangeApply", GirlNameIAT, "buy_dress")))
...
label IntAmandaDressChangeApply(GirlNameIAT="amanda", choice_code=""):
    ...
    if str(choice_code or "") == "offer_bra":
        $ MainTxt = '"Амандочка, милашка, а зачем же ты ходишь в лифчике?" - неожиданно спросили вы сестренку. "Ведь если начистоту, то он тебе пока не нужен, более того, он скрывает твою хоть и небольшую, но красивую и упругую грудь. Если ты будешь ходить без него, то платье лучше подчеркнет все ее изумительные изгибы если оно будет надето на голое тело," начали вы вешать ей лапшу на уши.'
        if DressPartSlut.get(topdress.get(GirlNameIAT, ""), 0) < 4:
            if sluttiness.get(GirlNameIAT, 0) < 35:
                ...
                $ AgreedToRedress = 1
                if sluttiness.get(GirlNameIAT, 0) < 50:
                    if renpy.random.randint(1, 2) == 1:
                        ...
                        $ MainTxt += '\n\nАманда и вы спрятались за стойкой, где вас не было видно из зала. Вы запустили руки под платье сестры, расстегивая многочисленные завязки и крючочки. Минута работы - и трофей уже в ваших руках. Правда ненадолго, сестра тут же забрала его обратно и спрятала.'
                else:
            ...
            if sluttiness.get(GirlNameIAT, 0) < 50:
                ...
                $ MainTxt += '"\n"Ну, наверное ты прав," после небольшого колебания согласилась с вами сестра, "помоги тогда."\nВы не замедлили оказать ей братскую помощь, запустив руки под платье, расстегнув и затем вытащив лиф. Лишь потом вы поняли что даже и не попробовали отойти куда в сторонку.'
                $ AgreedToRedress = 1
        ...
        if str(_other_saw or "").strip() != "":
            $ MainTxt += "\n\n" + str(_other_saw)
        $ Talked[GirlNameIAT] = Talked.get(GirlNameIAT, 0) + 1
        $ CurLocDesc = MainTxt
        call IntAmandaDressChangeRefresh(GirlNameIAT)
        return
    ...
    if str(choice_code or "") == "offer_panties":
        ...
        if CurrentLoc.get("liza", "") == "TavernMain":
            ...
            if pantiesdef.get("liza", "") == "":
                ...
                $ tmpLizaComandoBonus = min(10, int(AmandaVar.get("lizafriends", 0) or 0) // 2)
            else:
                $ MainTxt += '\n\n"Лишь бы чего ляпнуть," укоризнено сказала вам сестра, "я прекрасно знаю что это не так."'
                $ tmpLizaComandoBonus = -min(10, int(AmandaVar.get("lizafriends", 0) or 0) // 4)
        else:
            $ MainTxt += '"'
        if DressPartSlut.get(bottomdress.get(GirlNameIAT, ""), 0) < 4:
            if sluttiness.get(GirlNameIAT, 0) + tmpLizaComandoBonus < 42:
                ...
                $ MainTxt += '\n\n"Ну, может оно и так," сказала сестренка. "Ведь никто ничего не увидит, значит все в порядке. Может и буду я шустрей работать. Надо попробовать."'
                $ AgreedToRedress = 1
                if sluttiness.get(GirlNameIAT, 0) < 50:
                    if renpy.random.randint(1, 2) == 1:
            ...
            if sluttiness.get(GirlNameIAT, 0) + tmpLizaComandoBonus < 55:
                $ MainTxt += '\n\n"Да ты издеваешься, братец?!," возмутилась Аманда в ответ на ваше предложение. "Ты мое платье видел? Если не видел, то вот, посмотри! Оно же попу едва закрывает! Если я наклонюсь, то всем посетителям свою девочку покажу. Какое уж тут, прости Ильматер, обслуживание, если все меня будут лапать. А лапать будут, ты сам наших посетителей знаешь!" '
            else:
                $ MainTxt += '\n\n"Эх братик, братик," пошло улыбнулась Аманда "Ты ведь прекрасно знаешь, что в таком коротком платьичке, да без панталончиков, каждый мою писю увидит. А ведь нагибаться придется, там со стола вытри, здесь кружки поставь. Это ты называешь лучшим обслуживанием?"\n"Именно это, ты все отлично поняла," не смутились вы.\n"Ой, уговорил, языкастый, попробуем!" с готовностью отозвалась ваша еще недавно невинная сестренка.\nДаже не попытавшись уйти за стойку или еще куда, Аманда сунула руки под короткую юбочку и стянула с себя панталончики, показав на секунду свою щелку.'
                $ AgreedToRedress = 1
        if AgreedToRedress == 1:
    ...
    if str(choice_code or "") == "shame_bra":
        $ MainTxt = '"Аманда, а не стыдно тебе без лифа ходить?" - пристыдили вы сестру. "Половина посетителей на тебя пялится!"'
        if str(bra.get(GirlNameIAT, "") or "") != "" or (str(bra.get(GirlNameIAT, "") or "") == "" and sluttiness.get(GirlNameIAT, 0) >= 50 and renpy.random.randint(1, 2) == 1):
            $ MainTxt += '\n\n"А с чего это ты решил что нету? Как это ты проверил? Во сне меня что ли без лифа представил?" пресекла ваши поучения Аманда. '
        else:
            $ MainTxt += '\n\n"Ну да, ты же меня сам так надоумил," обескураженно ответила вам она.\n"Сказал что мол изгибы мои изумительные так лучше будут подчеркнуты, помнишь?"\n"Не помню!" Отрезали вы. "А если и сказал, то уже сожалею, погорячился я, как-то уж слишком пошло ты без него выглядишь, а ты как-никак моя сестра."'
            if sluttiness.get(GirlNameIAT, 0) < 45:
                $ MainTxt += '\n\n"Ну, коль ты теперь по другому заговорил, то пойду и одену его обратно. Я себя и правда как-то неловко чувствовала"'
                $ AgreedToRedress = 1
            elif sluttiness.get(GirlNameIAT, 0) < 60 and renpy.random.randint(1, 4) <= 3:
                ...
                $ MainTxt += '\n\n"Ай-яй, братик, значит ты у нас теперь скромник?!" засмеялась Аманда. "Ну а я, представь себе, нет! И я теперь уже взрослая и могу сама выбирать как мне одеваться! Так то!"'
    ...
    if str(choice_code or "") == "shame_panties":
        $ MainTxt = '"Аманда, мне показалось что у тебя под юбкой ничего нет?" - пожурили вы сестренку-шалопайку. "И не стыдно?"'
        if str(panties.get(GirlNameIAT, "") or "") != "" or (str(panties.get(GirlNameIAT, "") or "") == "" and sluttiness.get(GirlNameIAT, 0) >= 45 and renpy.random.randint(1, 2) == 1):
    ...
    if str(choice_code or "") == "buy_dress":
        $ MainTxt = '"Амандочка, а хочешь я тебе обновку куплю?" - задали вы сестренке вопрос, хотя уже заранее знали на него ответ.\n"Конечно хочу!" Аманда даже подпрыгнула от радости и еле удержалась чтобы не захлопать в ладоши.\n"Ну тогда завтра, с утра пораньше, дуй к Ирме Фараго, я буду тебя там ждать, вместе и выберем!" заверили вы сестренку.'
