init 3 python:
    ...
    def bat_repellent_recipe_unlocked():
        return int(MelissaVar.get("bat_recipe_unlocked", 0) or 0) == 1
    ...
    def melissa_bat_breakfast_ready():
        return (
            ...
            and int(dayspassed or 0) >= int(MelissaVar.get("storage_rat_last_help_day", -1) or -1) + 2
            and not bool(BreakfastToday)
        )

    def melissa_night_noise_ready():
        return (
            str(CurLoc or "") == "TavernUpstairs"
            and int(time or 0) >= 4
            and melissa_bats_stage() == 1
        )
...
label MelissaNightNoiseScene:
    if not melissa_night_noise_ready():
...
label MelissaFindDrawingsScene:
    ...
    $ MainTxt = "Утром вы замечаете, как Мелисса, стоя на коленях у кровати Аманды, поспешно шарит рукой под самым низом, будто ищет что-то, что очень не хотелось бы показывать чужим глазам. Услышав шаги, она вздрагивает, краснеет и отстраняется слишком резко.\n\nКогда она уходит, вы все же проверяете, что там было, и находите несколько спрятанных под кроватью непристойных рисованных листков. Манера уверенная, линии смелые, а сюжеты вовсе не девичьи. Похоже, это не просто чужая пошлость, а чей-то тайный заработок — или по крайней мере опасная забава."
    $ CurLocDesc = MainTxt
    call TavernAmandaRoomBuildActions
    $ _paged_text = str(MainTxt or "")
    call QueuePagedPanelText(_paged_text, current_action_title, current_action_items, "plain")
