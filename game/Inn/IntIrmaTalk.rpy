init python:
    def irma_tailor_wolf_skin_count():
        return int(_player_item_count_by_id("wolf_skin_001") or 0)
    ...
    def irma_make_warm_cloak():
        ...
        if irma_tailor_bear_fur_count() >= 1:
            if int(irma_remove_best_bear_fur(1) or 0) != 1:
                return False
        elif not _player_remove_item_by_id("wolf_skin_001", 2):
            return False
    ...
    def irma_make_fur_bedding():
        ...
        if irma_tailor_bear_fur_count() >= 2:
            ...
            if int(irma_remove_best_bear_fur(1) or 0) != 1:
                return False
            if not _player_remove_item_by_id("wolf_skin_001", 2):
                return False
        elif not _player_remove_item_by_id("wolf_skin_001", 4):
            return False
...
label IntIrmaTalkRefresh:
    ...
    $ current_action_items.append(MenuItem("Закончить разговор", Call("DressShopRestore")))
    return
