...
label _int_becky_talk_smalltalk(girl_name="becky"):
    ...
    if Talked.get(girl_name, 0) <= 2 and renpy.random.randint(1, 2) == 1 and Friends.get(girl_name, 0) < 3:
...
label _int_becky_talk_personal(girl_name="becky"):
    ...
    if Talked.get(girl_name, 0) <= 2 and renpy.random.randint(1, 2) == 1 and Friends.get(girl_name, 0) <= 5:
...
label _int_becky_talk_inga1(girl_name="becky"):
    ...
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    $ BeckyVar["SawIngaFuck"] = 2
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_inga2(girl_name="becky"):
    ...
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    $ BeckyVar["SawIngaFuck"] = 3
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_lucas(girl_name="becky"):
    ...
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    $ BeckyVar["SawIngaFuck"] = 4
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_husband1(girl_name="becky"):
    ...
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    $ BeckyVar["husbandtalk"] = 2
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_husband2(girl_name="becky"):
    ...
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    $ BeckyVar["husbandtalk"] = 3
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_husband3(girl_name="becky"):
    ...
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    $ BeckyVar["husbandtalk"] = 4
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_husband4(girl_name="becky"):
    ...
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    $ BeckyVar["husbandtalk"] = 5
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_eddie1(girl_name="becky"):
    "Бекки явно гордилась своим сыном:"
    "\"Он у меня весь в папочку пошел! Девицам окрестным прохода не дает, а они ему! А однажды ко мне подруга моя зашла, Нора, а меня дома тогда не было. Прихожу - а Эдди ей уже юбку завернул и сношает. Не смогла устоять, как она мне сказала. Вот какой мой мальчик неотразимый!\" - похвасталась вам Бекки. Вы про себя подумали что дело могло быть не только в неотразимости Эдди, но и в том что эта самая Нора довольно страшновата. Но, по здравом размышлении, вы решили не делится этим соображением с Бекки."
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    $ BeckyVar["eddietalk"] = 1
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_eddie2(girl_name="becky"):
    ...
    $ tmpRnd = renpy.random.randint(1, 3)
    if tmpRnd == 1:
    ...
    call SlutFriendsIncrease(girl_name, 15, 1, 1, 0, 0, 0)
    if BeckyVar.get("eddietalk", 0) < 2:
        $ BeckyVar["eddietalk"] = 2
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_invite(girl_name="becky"):
    ...
    if BeckyVar.get("husbandtalk", 0) == 0:
        "\"Да нет, Стефанчик, я же тебя почти совсем не знаю, люди еще увидят, говорить будут, не, извини, но нет!\" - ответила вам вдова."
    else:
        $ InvitePoints = 0
        "\"Стефанчик, я бы и рада тебя пригласить, но что мои дети скажут, как я им тебя представлю? Они же не маленькие, все поймут. Вот Эдди, сыночек мой, как я ему в глаза буду смотреть?\" - спросила вас вдова."
        if BeckyVar.get("eddietalk", 0) >= 2:
            "\"Вот именно что не маленькие\", ответили вы не растерявшись."
            "\"Я же тебе рассказывал, что сыночек твой в моем трактире девиц снимает. Что ему, если мамка своего любовника к себе приведет, не удивишь его этим.\""
            $ InvitePoints += 1
        else:
            "Вы не нашлись с ответом и промолчали."
        "\"А дочка моя старшая, Ингеборг, что я ей скажу?\" - смотря вам в глаза спросила Бекки."
        if BeckyVar.get("SawIngaFuck", 0) >= 4:
        ...
        if BeckyVar.get("husbandtalk", 0) >= 3:
        ...
        if InvitePoints >= 4:
            ...
            if BeckyVar.get("visitedhome", 0) < 3:
...
label _int_becky_talk_lastvisit(girl_name="becky"):
    if BeckyVar.get("visitedhome", 0) >= 6:
        "\"Бекки, как тебе завершение ужина?\" - осведомились вы."
        if renpy.random.randint(1, max(1, 3 + BeckyVar.get("BeckyOpenMinet", 0))) == 1:
            "\"А ты заметил, что Ингенборг, доча моя, совсем большая стала?\" ответила вам мать семейства."
        elif BeckyVar.get("BeckyOpenMinet", 0) > 0:
            if sluttiness.get("becky", 0) < 48:
                "\"Ну я не знаю, все-таки как-то неловко мне было на глазах сына и дочки.\""
            else:
                $ RandVar = renpy.random.randint(1, 4)
                if RandVar == 1:
                    "\"Неплохо, но ты заметил, Стефан, как на меня Эдди смотрел? Даже, пожалуй почаще чем на свою Жоржетту. Не такая я уж и старая видно, что на меня не только ты, но даже и сыночек мой так смотрит.\" гордо заявила вам в ответ вдова."
                elif RandVar == 2:
        ...
        if BeckyVar.get("TimesVisited", 0) <= 2:
            ...
            if renpy.random.randint(1, 2) == 1:
                ...
                "\"А ты заметил, что Ингенборг, доча моя, совсем большая стала?\" ответила вам мать семейства."
        elif BeckyVar.get("HomeSex", 0) == 0 and BeckyVar.get("visitedhome", 0) >= 5:
            "\"А ты заметил, что Ингенборг, доча моя, совсем большая стала?\" ответила вам мать семейства."
        else:
...
label _int_becky_talk_eddie3(girl_name="becky"):
    "\"Дорогая, а ты заметила как на тебя смотрит твой собственный сын?\" невзначай осведомились вы. \"Кажется, что у него встает при одной мысли о тебе.\""
    if sluttiness.get("becky", 0) >= 50:
        if BeckyVar.get("TalkAboutEddie", 0) == 0:
            "\"Заметила, как же не заметить, ведь я его мама,\" отозвалась Бекки. \"Даже не знаю, что и делать. Вижу что он мучается, мне как матери это больно видеть. Но он же мой сын.Хорошо, что он себе какую-то женщину нашел, моего возраста, может хоть это его успокоит.\""
        else:
            "\"Так я же уже сказала, что заметила, я же не слепая.\""
    else:
        if BeckyVar.get("TalkAboutEddie", 0) == 0:
            "\"Заметила, как же не заметить, ведь я его мама,\" отозвалась Бекки. \"Стыд-то какой и позор. Будь он помладше, выпорола бы его как сидор свою козу. А сейчас вон он какой вымахал, а ни ума, ни совести. Даже и говорить об этом не хочу.\""
        else:
            "\"Я тебе уже отвечала, не хочу это даже обсуждать.\""
            call SlutFriendsIncrease("becky", 10, 3, -1, 35, 3, -1)
    $ BeckyVar["TalkAboutEddie"] = 1
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_eddie4(girl_name="becky"):
    "\"Бекки, дорогуша, я тут наблюдал преинтереснейшую сценку,\" как бы между делом упомянули вы. \"Представляешь, твой сыночек трахал эту свою Жоржетту.\""
    "\"И что?\" удивленно подняла бровь Бекки."
    "\"Так вот, он ее попросил называть его сыночком, а сам он Жоржетту называл мамочкой и Бекки. Прямо как тебя зовут. Вот такая у твоего отпрыска бурная фантазия.\""
    $ BeckyVar["GeorgMention"] = 1
    if sluttiness.get("becky", 0) <= 35:
        ...
        if renpy.random.randint(1, 2) == 1:
            "\"Ох стервец,\" засмеялась Бекки, \"Прямо как Эрик, мой покойный муженек, тот тоже говорил, что он пару раз своей мамочке засадил, до сих пор не знаю, правда ли это или придумал он все.\""
        else:
            "\"Эх, затейник,\" ответила вам разбитная Эддина мамашка, \"Значит не стара я еще, что на меня мальчики заглядываются. Сначала ты, а теперь и сыночек мой обо мне фантазирует.\""
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
    return


label _int_becky_talk_eddie5(girl_name="becky"):
    "\"Да твой Эдди наглец, свинья неблагодарная, онанист недоделанный!\" в гневе закричали вы, \"Да как он только посмел так смотреть на собственную мать!\""
    $ ChangeMind = 0
    if BeckyVar.get("EddieIntrReact", 0) == 2:
        $ ChangeMind = 1
    if BeckyVar.get("EddieIntrReact", 0) == 1:
        $ ChangeMind = 2
    $ BeckyVar["EddieIntrReact"] = 1
    if sluttiness.get("becky", 0) <= 35:
        "\"Стефан, как же ты все правильно говоришь\", поддакнула вам Бекки, \"вот так и расти их, а вырастут - и никакого уважения к матери.\""
        if ChangeMind < 2:
            call SlutFriendsIncrease("becky", 15, 2, 1, 35, 2, -1)
    elif sluttiness.get("becky", 0) <= 52:
        ...
        if ChangeMind < 2:
            call SlutFriendsIncrease("becky", 0, 0, 0, 35, 2, -1)
    else:
        "\"Да ладно тебе, Стефан, ты наверное просто ревнуешь,\" возразила вам вдова. Кажется, вы задели ее материнские чувства. \"Не смей так говорить об Эдди, он все-таки мой сын.\""
        call SlutFriendsIncrease("becky", 9, 1, -1, 55, 2, 1)
    if ChangeMind == 1:
...
label _int_becky_talk_eddie6(girl_name="becky"):
    "\"Бекки, любимая, раз Эдди так по тебе сохнет, может не стоит его мучать?\" заявили вы, демонстрируя широту своих взглядов. \"Ну и что, что он твой сын, он уже повзрослел и видит в тебе женщину, а не мать. Не смотри на него, как на маленького.\""
    $ ChangeMind = 0
    if BeckyVar.get("EddieIntrReact", 0) == 2:
        $ ChangeMind = 2
    if BeckyVar.get("EddieIntrReact", 0) == 1:
        $ ChangeMind = 1
    $ BeckyVar["EddieIntrReact"] = 2
    if sluttiness.get("becky", 0) <= 35 and BeckyVar.get("PriestAdvice", 0) == 0:
        "\"Так,\" гневно сказала вдова, отступая от вас на шаг. \"Это мне показалось, или ты только что предложил мне заняться любовью с собственным сыном?! Хорошо же ты обо мне думаешь. Ну ты и подонок, Стефан! Вон из моей лавки!\""
        "\"Да, надо было держать язык за зубами\" - эту мудрую мысль вы додумали уже на улице."
        call SlutFriendsIncrease("becky", 5, 1, -1, 25, 1, -1)
        jump MarketPlace
    elif sluttiness.get("becky", 0) <= 48:
        ...
        if ChangeMind < 2:
            call SlutFriendsIncrease("becky", 12, 4, -1, 35, 4, -1)
    else:
        if BeckyVar.get("PriestAdvice", 0) <= 1:
            if pregnancy.get("becky", 0) > 60:
                "\"Ты так думаешь?\" спросила Бекки, \"я над этим уже думала, я сейчас в тягости и второй раз забеременеть мне не грозит. Но все-таки Эдди мой сын, не могу я на такой грех пойти. Может с отцом Герхардом пойти посоветоваться, что он думает...\" - вдова уже разговаривала сама с собой."
                if ChangeMind < 2:
                    call SlutFriendsIncrease("becky", 17, 2, 1, 56, 1, 1)
                $ BeckyVar["PriestAdvice"] = 1
            elif sluttiness.get("becky", 0) <= 52:
                "\"Ты так думаешь?\" спросила Бекки, \"я над этим уже думала, но я так не могу. А если залечу от собственного сына? Сестренка моя непутевая, Грета, правда вроде однажды так залетела, но я не она, нет, стыд-то какой, даже и не говори об этом.\""
                if ChangeMind < 2:
                    call SlutFriendsIncrease("becky", 17, 2, 1, 54, 1, 1)
            else:
                "\"Ты так думаешь?\" спросила Бекки, \"даже и не знаю, вдруг я от собственного сына залечу? Сестренка моя непутевая, Грета, правда вроде однажды так залетела, но все-таки это грех. А с другой стороны Эдди мучается, а я только о себе и думаю. Даже и не знаю что делать. Может с отцом Герхардом пойти посоветоваться, что он думает...\" - вдова уже разговаривала сама с собой."
                if ChangeMind < 2:
                    call SlutFriendsIncrease("becky", 17, 2, 1, 58, 1, 1)
                $ BeckyVar["PriestAdvice"] = 1
            ...
            "\"Даже и не знаю, отец Герхард говорит что ничего такого в том, что я отдамся сыну, нет, но как мне Эдди об этом сказать?\" - поделилась вдова своими сомнениями."
        call SlutFriendsIncrease("becky", 9, 1, -1, 55, 2, 1)
    if ChangeMind == 1:
        "\"Хотя, погоди-ка, раньше же ты совсем по другому говорил, разве нет?\" с подозрением заметила миссис Блэнкеншип."
    $ Talked[girl_name] = Talked.get(girl_name, 0) + 1
...
label _int_becky_talk_eddie7(girl_name="becky"):
    if BeckyVar.get("visitedhome", 0) < 7:
        "\"И как тебе прошлая ночь с Эдди?\" осведомились вы у Ребекки."
        "\"Ох, даже не знаю что на меня нашло,\" ответила вам почтенная матрона. \"Дать собственному сыну?! А если я от него затяжелею? Я ведь не такая, как моя непутевая сестрица Грета, нет. Ну и подставил же ты меня, Стефан!\" упрекнула вас скромница Бекки."
        "\"Эй, да ведь ты сама этого хотела,\" возразили вы. \"Я же видел как ты текла когда Эдди тебя трахал.\""
        "\"Ну и что? Один раз слабость стоило проявить так теперь ты меня этим попрекать будешь? Нетушки, одного раза вполне хватит пока.\" - отрезала вдова."
        "\"Надо будет еще со святым отцом посоветоваться,\" вполголоса добавила она."
        $ BeckyVar["AskedEddieFuck"] = 1
    else:
