    ...
    class TownStreetRuntime(object):
        ...
        TIME_EVENTS = {
            "morning": (
                ...
                {"hooks": ("vampire", "witch", "corpse"), "text": "Утренний туман на [улица] скрывает страшную находку. [имя] - [занятие] наткнулся на полностью обескровленное тело девушки. «Опять упырь из леса! Или та ведьма, что сбежала с костра прошлой весной!» - кричит он. Прохожие крестятся, а стражники только пожимают плечами и пьют из фляжки."},
                {"hooks": ("stocks", "moral", "shame"), "text": "У позорного столба на [улица] стоит голая по пояс [имя] - [занятие], с табличкой «За блуд и содомию». Толпа плюет, бросает гнилые овощи и камни. Рядом глашатай объявляет: «Так будет со всяким, кто нарушит чистоту нравов!»"},
                ...
                {"hooks": ("black_dog", "curse", "abduction"), "text": "Молодая [имя] бежит по [улица], юбки в крови, кричит: «Он вышел из леса! Черный пес с горящими глазами! Утащил мою сестру!» Толпа крестится и шепчет о старом проклятии, что легло на город после той алхимической вспышки в прошлом году."},
                {"hooks": ("plague", "potion", "ghost"), "text": "На [улица] [имя] - [занятие] продает «чудодейственное» зелье от французской болезни. «Пейте, пока чума не вернулась!» - орет он. В толпе шепчутся, что вчера ночью видели призрак повешенной ведьмы именно на этой улице."},
            ),
            "noon": (
                ...
                {"hooks": ("confession", "demon", "sin"), "text": "В полдень на [улица] [имя] - [занятие] стоит на коленях перед женой и умоляет простить за то, что «продал душу лесному демону за одну ночь с блудницей». Толпа смеется, но многие крестятся - слухи о договорах с нечистью ходят уже второй месяц."},
                ...
                {"hooks": ("public_sin", "plague", "guards"), "text": "На [улица] толстый [имя] - [занятие] публично совокупляется с служанкой прямо у стены, не стесняясь толпы. «Пусть смотрят! После чумы жизнь коротка!» - орет он. Стражники проходят мимо и только ржут."},
            ),
            "weekends": (
                ...
                {"hooks": ("balcony", "black_beast", "forest"), "text": "Выходной хаос на [улица]: [имя] показывает голую задницу с балкона, а друзья закидывают его гнилыми овощами. Вдруг из леса доносится вой - все замолкают. «Опять он... черный зверь», - шепчут люди."},
            ...
            "evening": (
                ...
                {"hooks": ("bandits", "message", "witch"), "text": "Вечером на [улица] [имя] - [занятие] находит у порога отрезанную голову своего брата. «Это послание от лесных разбойников... или от той ведьмы», - шепчет он. Толпа крестится."},
                {"hooks": ("music", "dance", "ghost"), "text": "На площади играет скрипач. [имя] - [занятие] крутит партнершу так, что корсаж лопается. Но вдруг музыка обрывается - кто-то кричит: «Призрак! Призрак повешенной ведьмы!» Все разбегаются."},
                ...
                {"hooks": ("horse", "forest_shadow", "curse"), "text": "Сумерки на [улица]: [stallion] вырывается и несется в лес. За ним - черная тень. Хозяин [имя] - [занятие] падает на колени: «Он забрал его... лес забрал!»"},
                {"hooks": ("bribe", "plague", "street_sin"), "text": "Вечерний свет на [улица]. [имя] - [занятие] сует монеты служанке, пока муж смотрит. «После заката все позволено», - смеется он. В толпе шепчутся о чуме, которая якобы вернулась."},
            ),
            "night": (
                ...
                {"hooks": ("murder", "paw_tracks", "dog"), "text": "В ночной тишине на [улица] [имя] - [занятие] валяется в канаве, штаны спущены, горло перерезано. Рядом - следы огромных лап. Собака обнюхивает кровь."},
        ...
        def _call_name(self, gender=None):
            ...
            if callable(fn):
                return fn(gender=gender or renpy.random.choice(["male", "female"]), nationality=renpy.random.choice(["German", "French", "Italian"]))
            return renpy.random.choice(["Ганс", "Мария", "Пьер", "Лючия"])

        def _call_occupation(self):
            fn = self._get("RandomOccupCode", None)
            if callable(fn):
                return fn()
            return renpy.random.choice(["подмастерье", "торговец", "возчик", "служанка"])
        ...
        def random_chronicle(self, time_of_day="morning"):
            ...
            return (
                ...
                .replace("[имя]", self._call_name())
                .replace("[занятие]", self._call_occupation())
        ...
        def apply_cloth_damage(self, amount=15):
            globals()["costumecondition"] = max(0, self._int(self._get("costumecondition", 100), 100) - self._int(amount, 15))
        ...
        def make_help_context(self):
            ...
            ctx["help_name"] = self._call_name()
            ctx["help_job"] = self._call_occupation()
            return ctx
