...
label ChurchServiceLegare:
    ...
    vscene "images/Alber/church/household_school.png"
    $ scene_runtime.text = "Мессир Легаре в черном камзоле стоит около колонны и внимательно слушает службу. Рядом его жена Элоиза, маленькая шатенка средних лет, профессиональная гувернантка и хозяйка пансиона в их доме. Позади видны Жерар, Жан-Жак и Реми"
    $ scene_runtime.text += "." if Clara.mongol_case_detained() else "; Кларисса стоит чуть в стороне."
    $ scene_runtime.text += " Поблизости от Элоизы — Полина, взрослая воспитанница пансиона из маленькой далекой деревни, а не дочь Легаре. Она старается держаться чинно, время от времени поглядывая на наставницу."
    $ scene_runtime.location_text = scene_runtime.text
    show screen main_ui
    menu:
        "Рассмотреть Элоизу и Полину":
            vscene "images/Alber/church/aloise_pauline_school.png"
            $ scene_runtime.text = "Элоиза держится с привычной уверенностью женщины, привыкшей учить других хорошим манерам. Заметив неловкость Полины, она едва заметным движением показывает, как держать молитвенник. Девушка поправляет руки и выпрямляется. Даже здесь, среди колонн и свечей, обучение не прекращается; однако о распорядках этого пансиона лучше расспросить Клариссу наедине."
            menu:
