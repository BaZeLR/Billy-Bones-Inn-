...
def test_tavern_main_closed_state_uses_clock_hours_not_day_slot():
    ...
    assert "return int(week or 0) == 7" in source
    assert "return int(week or 0) != 7 and not tavern_main_late_closed()" in source
    assert "def tavern_main_friday_dance_closed()" in source
    assert "18 <= int(hour or 0) < 22" in source
    assert "$ TavernClosed = tavern_main_closed_text()" in source
...
def test_tavern_preopening_is_before_noon_by_clock():
    ...
    assert "6 <= int(hour or 0) < 12" in preopening
...
def test_tavern_main_sets_day_night_room_picture_explicitly_for_main_ui():
    ...
    assert 'scene_image = "images/tavern/mainhall/main_hall_night.png" if int(calendar_v2.hour or 0) >= 18 or int(calendar_v2.hour or 0) < 6 else "images/tavern/mainhall/main_hall.png"' in source
