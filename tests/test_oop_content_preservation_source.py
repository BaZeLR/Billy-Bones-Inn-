...
def test_authored_reconciliation_paths_remain_reachable():
    ...
    assert '"Попробовать помириться с Мелиссой" if Melissa.can_apologize():' in melissa
    ...
    assert "= Melissa.attempt_apology()" in melissa
    assert "call SlutFriendsIncrease(girl_name, 6, 1, 1, 0, 0, 0)" not in melissa
    ...
    assert '"Попробовать помириться с Сандрой" if Sandra.can_apologize():' in sandra
    ...
    assert "= Sandra.attempt_apology()" in reconcile
    assert "procedural_randint" not in reconcile
