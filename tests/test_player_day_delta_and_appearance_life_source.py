...
def test_player_appearance_owns_wash_haircut_and_life_timers():
    ...
    assert "self.days_since_wash = 0" in source
    assert "self.days_since_haircut = 0" in source
    assert "self.washDays" not in source
...
def test_next_day_ages_player_appearance_once_per_day():
    ...
    assert "player.daily_maintenance(1)" in source
